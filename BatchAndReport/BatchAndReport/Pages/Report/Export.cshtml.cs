...
namespace BatchAndReport.Pages.Report
{
    public class ExportModel : PageModel
    {
        ...
        {
            // 1. Locate the source Word document
            var docPath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "document", "Testcontract.docx");
            if (!System.IO.File.Exists(docPath))
                return NotFound("Source Word document not found.");

            // 2. Load the Word document into memory
            byte[] wordBytes = System.IO.File.ReadAllBytes(docPath);

            // 3. Convert Word to PDF using QuestPDF (requires a library that supports conversion, e.g., DocXToPdf or similar)
            // QuestPDF does not natively convert DOCX to PDF, so use OpenXmlPowerTools to extract text, then render with QuestPDF.
            // For demonstration, extract plain text and render as PDF.

            string extractedText;
            using (var memStream = new MemoryStream(wordBytes))
            ...
            QuestPDF.Fluent.Document.Create(container =>
            {
                container.Page(page =>
                {
                    page.Content().Text(extractedText ?? "No content found.");
                });
            }).GeneratePdf(pdfStream);

            pdfStream.Position = 0;
            return File(pdfStream, "application/pdf", "Testcontract.pdf");
        }
