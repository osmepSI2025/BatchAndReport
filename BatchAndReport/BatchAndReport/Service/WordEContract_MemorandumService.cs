...
//        // --- ส่วนที่ปรับปรุงใหม่: สร้าง HTML สำหรับโลโก้หลายรูป ---
//        string partnershipLogosHtml = "";
//        if (!string.IsNullOrEmpty(result.Organization_Logo) && result.Organization_Logo.Contains("<content>"))
//        {
//            try
//            {
//                var logoDirectory = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "images", "logox");
//                var partnershipHtmlBuilder = new StringBuilder();

//                // เริ่มต้นตารางย่อยสำหรับโลโก้คู่ค้า
//                partnershipHtmlBuilder.Append("<table style='width: 100%; border-collapse: collapse;'><tr>");

//                // วนลูปเพื่อค้นหาและอ่านไฟล์ .png ทั้งหมดในโฟลเดอร์
//                var logoFiles = Directory.GetFiles(logoDirectory, "*.png").OrderBy(f => f);
//                foreach (var filePath in logoFiles)
//                {
//                    var bytes = System.IO.File.ReadAllBytes(filePath);
//                    var base64String = Convert.ToBase64String(bytes);
//                    // เพิ่มแต่ละโลโก้ในเซลล์ของตาราง
//                    partnershipHtmlBuilder.Append($"<td style='padding: 5px; text-align: center;'><img src='data:image/png;base64,{base64String}' height='40' /></td>");
//                }

//                // เพิ่มลูกศรในเซลล์สุดท้าย
//                if (logoFiles.Count() > 1)
//                {
//                    partnershipHtmlBuilder.Append("<td style='padding: 5px; text-align: center; vertical-align: middle;'>");
//                    partnershipHtmlBuilder.Append("<span style='font-size: 20pt;'>-></span>");
//                    partnershipHtmlBuilder.Append("</td>");
//                }

//                partnershipHtmlBuilder.Append("</tr></table>");
//                partnershipLogosHtml = partnershipHtmlBuilder.ToString();
//            }
//            catch (Exception ex)
//            {
//                Console.WriteLine($"Error reading logo files: {ex.Message}");
//                partnershipLogosHtml = "";
//            }
//        }
//        // --- สิ้นสุดส่วนปรับปรุงใหม่ ---
...
//        var signatoryTableHtml = signlist.Count > 0 ? await _eContractReportDAO.RenderSignatory(signlist) : "";
//        var signatoryTableHtmlWitnesses = signlist.Count > 0 ? await _eContractReportDAO.RenderSignatory_Witnesses(signlist) : "";
//        #endregion signlist
...
//                <img src='data:image/jpeg;base64,{logoBase64}' width='150' height='50' />
//            </div>
//        </td>
    ...
    <body>
     {contractLogoHtml}
       ...
       <div class='t-16 text-center'><B>โครงการ {result.ProjectTitle}</B></div>
        ...
        <div class='t-12 text-center'><B>สำนักงานส่งเสริมวิสาหกิจขนาดกลางและขนาดย่อม</B></div>
        <div class='t-12 text-center'><B>กับ</B></div>
        <div class='t-12 text-center'><B>{result.OrgName ?? ""}</B></div>
        <br/>
         <P class='t-12 tab2'>บันทึกข้อตกลงความร่วมมือฉบับนี้ทำขึ้น ณ สำนักงานส่งเสริมวิสาหกิจขนาดกลาง และขนาดย่อม เมื่อ {strSign_Date} ระหว่าง</P>
        <P class='t-12 tab2'><B>สำนักงานส่งเสริมวิสาหกิจขนาดกลางและขนาดย่อม</B>  โดย {result.OSMEP_NAME} ตำแหน่ง {result.OSMEP_POSITION} {strAttorneyOsmep} สำนักงานตั้งอยู่เลขที่ 120 หมู่ 3 ศูนย์ราชการเฉลิมพระเกียรติ 80 พรรษา 5 ธันวาคม 2550 (อาคารซี) ชั้น 2, 10, 11 ถนนแจ้งวัฒนะ แขวงทุ่งสองห้อง เขตหลักสี่ กรุงเทพ 10210 ซึ่งต่อไป ในสัญญาฉบับนี้จะเรียกว่า“สสว.” ฝ่ายหนึ่ง กับ</P>
        <P class='t-12 tab2'>“{result.OrgCommonName ?? ""}” {result.CP_S_NAME} ตำแหน่ง {result.CP_S_POSITION} {strAttorney} สำนักงานตั้งอยู่เลขที่ {result.Office_Loc} ซึ่งต่อไปในสัญญาฉบับนี้จะเรียกว่า “{result.OrgName ?? ""}” อีกฝ่ายหนึ่ง</P>
