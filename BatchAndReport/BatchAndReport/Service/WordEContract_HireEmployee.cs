...
EContractDAO eContractDAO
    ...
    public async Task<string> OnGetWordContact_HireEmployee_ToPDF(string id, string typeContact)
    {
        try
        {
            ...
            string strAttorneyLetterDate = CommonDAO.ToThaiDateStringCovert_Noname(result.AttorneyLetterDate ?? DateTime.Now);
            string strAttorney =
                result.AttorneyFlag == true
                ? $"ผู้รับมอบหมายตามคำสั่งสำนักงานฯ ที่ {result.AttorneyLetterNumber ?? ""} ลง{strAttorneyLetterDate}"
                : "";

            string strcontractsign = CommonDAO.ToThaiDateStringCovert_Noname(result.ContractSignDate ?? DateTime.Now);
            string strHiringStart = CommonDAO.ToThaiDateStringCovert_Noname(result.HiringStartDate ?? DateTime.Now);
            string strHiringEnd = CommonDAO.ToThaiDateStringCovert_Noname(result.HiringEndDate ?? DateTime.Now);
...
<p class='tab2 t-12'>
   สัญญาฉบับนี้ทำขึ้น ณ สำนักงานส่งเสริมวิสาหกิจขนาดกลางและขนาดย่อม เลขที่ ๑๒๐ หมู่ ๓ ศูนย์ราชการเฉลิมพระเกียรติ ๘๐ พรรษา ๕ ธันวาคม ๒๕๕๐ (อาคารซี) ชั้น ๒, ๑๐, ๑๑ ถนนแจ้งวัฒนะ </br>แขวงทุ่งสองห้อง เขตหลักสี่ กรุงเทพ ๑๐๒๑๐
 เมื่อ {strcontractsign}
</p>

   

<p class='full-width-text tab2 t-12'>
ระหว่าง สำนักงานส่งเสริมวิสาหกิจขนาดกลางและขนาดย่อม โดย {CommonDAO.ConvertStringArabicToThaiNumerals(result.OSMEP_NAME) ?? ""}</p>
<p class='tab0 t-12'>
ตำแหน่ง {CommonDAO.ConvertStringArabicToThaiNumerals(result.OSMEP_POSITION) ?? ""} {CommonDAO.ConvertStringArabicToThaiNumerals(strAttorney)} 
ซึ่งต่อไปในสัญญานี้จะเรียกว่า <b>“ผู้ว่าจ้าง”</b>
</p>
<p class='full-width-text tab2 t-12'>
...
<p class='tab2 t-12'>
๒. ผู้ว่าจ้างจะจ่ายค่าจ้างให้แก่ลูกจ้างในระหว่างระยะเวลาการปฏิบัติงานของลูกจ้างตามสัญญานี้ 
ในอัตราเดือนละ {CommonDAO.ConvertCurrencyToThaiNumerals(result.Salary.HasValue ? (int)result.Salary.Value : 0)}  บาท ({strSalary}) 
โดยจะจ่ายให้ในวันทำการก่อนวันทำการสุดท้ายของธนาคารในเดือนนั้นสามวันทำการ และนำเข้าบัญชีเงินฝากของลูกจ้าง ณ ที่ทำการของผู้ว่าจ้าง หรือ ณ ที่อื่นใดตามที่ผู้ว่าจ้างกำหนด 
 ...
 <p class='full-width-text tab2 t-12'>
  ๓. ในการจ่ายค่าจ้าง และ/หรือ เงินในลักษณะอื่นให้แก่ลูกจ้าง ลูกจ้างตกลงยินยอมให้ผู้ว่าจ้าง หักภาษี ณ ที่จ่าย และ/หรือ เงินอื่นใดที่ต้องหักโดยชอบด้วยระเบียบ ข้อบังคับของผู้ว่าจ้างหรือตามกฎหมายที่เกี่ยวข้อง
</p>
<p class=' tab2 t-12'>
...
<p class='full-width-text tab2 t-12'>
 ๖. ตลอดระยะเวลาการปฏิบัติงานตามสัญญานี้ ลูกจ้างจะต้องปฏิบัติตามกฎ ระเบียบ ข้อบังคับ  
</br>คำสั่งหรือประกาศใด ๆ ของผู้ว่าจ้าง ตลอดจนมีหน้าที่ต้องรักษาวินัยและยอมรับการลงโทษทางวินัยของผู้ว่าจ้างโดยเคร่งครัด 
...
ทั้งนี้ ต้องรักษาผลประโยชน์และชื่อเสียงของผู้ว่าจ้าง และไม่เปิดเผยความลับหรือข้อมูล</p>
<p class=' tab0 t-12'>
ของทางราชการให้ผู้หนึ่งผู้ใดทราบ โดยมิได้รับอนุญาตจากผู้รับผิดชอบงานนั้นๆ 
</p>
<p class='full-width-text tab2 t-12'>
๘. ลูกจ้างมีหน้าที่ปฏิบัติงานที่เกี่ยวข้องกับการประมวลผลข้อมูลส่วนบุคคลไม่ว่าจะเป็นการเก็บ
รวบรวม การใช้และการเปิดเผยข้อมูลส่วนบุคคลโดยเคร่งครัดตามกฎหมายว่าด้วยการคุ้มครองข้อมูลส่วนบุคคล 
...
การฝ่าฝืนหน้าที่ดังกล่าวเป็นเหตุให้เจ้าของข้อมูลส่วน</p>
<p class=' tab0 t-12'>
บุคคลหรือผู้ว่าจ้างเสียหาย ถือเป็นการผิดสัญญาจ้างงานอย่างร้ายแรงที่ผู้ว่าจ้างมีสิทธิบอกเลิกสัญญาจ้างได้ทันที
</p>
<p class='tab2 t-12'>
...
</br>พ้นสภาพการเป็นลูกจ้างได้ทันที โดยไม่จำเป็นต้องมีหนังสือว่ากล่าวตักเตือน และผู้ว่าจ้างไม่ต้องจ่ายค่าชดเชย
...
<p class='full-width-text tab2 t-12'>
  ๑๒. ในกรณีที่ลูกจ้างกระทำการใดอันทำให้ผู้ว่าจ้างได้รับความเสียหาย ไม่ว่าเหตุนั้นผู้ว่าจ้างจะ</br>นำมาเป็นเหตุบอกเลิกสัญญาจ้างหรือไม่ก็ตาม ผู้ว่าจ้างมีสิทธิจะเรียกร้องค่าเสียหาย และลูกจ้างยินยอมชดใช้ค่าเสียหายตามที่ผู้ว่าจ้างเรียกร้องทุกประการ 
</p>
<p class='tab2 t-12'>
