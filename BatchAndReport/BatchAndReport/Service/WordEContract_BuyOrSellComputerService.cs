...
# region 4.1.1.2.9.สัญญาเช่าคอมพิวเตอร์ 
    ...
    //                htmlDocAtt += $"<p class='tab4 t-16'> {docItem.DocumentTitle} จำนวน {docItem.PageAmount} หน้า </p>";
    ...
    //                    // Use a single container for all signature info and set font-family
    //                    signatureHtml = $@"<img src='data:image/png;base64,{base64}' alt='signature' style='max-height: 80px;' />";
    //                }
    //                catch
    //                {
    ...
    //            // เพิ่มลงใน HTML, set font-family for the whole block
    //            signatoryHtml.AppendLine($@"
    //    <div class='t-16 text-right tab1' style='font-family: THSarabunNew, Arial, sans-serif;'>
    //        {signatureHtml}<br />
    //        ({name})<br />
    //        {role}
    //    </div>");
    ...
    //        {companySeal}      
    //    </p>
    //<p class='t-16 text-right tab1'>(ตราประทับ บริษัท)</p>

    //");
    //                }
    //                catch
    //                {
    //                    companySeal = "<p class='t-16 text-right tab1'>(ตราประทับ บริษัท)</p>";
    //                }
    //            }
    //        }
    //        #endregion  signlist
    ...
    //        var htmlContent = $@"
    //<div >
    //    <p class='text-center t-22'><b> แบบสัญญา</b></p>
    //    <p class='text-center t-22'><b> สัญญาซื้อขายคอมพิวเตอร์</b></p>
    //</br>
    //    <p class='text-right t-18'>สัญญาเลขที่ {(result.CPAContractNumber ?? "xxxxxx")}</p>
    //    <p class='tab4 t-16'>
    //        สัญญาฉบับนี้ทำขึ้น ณ สำนักงานส่งเสริมวิสาหกิจขนาดกลางและขนาดย่อม (สสว.) ตำบล/แขวง ทุ่งสองห้อง 
    //</br>อำเภอ/เขต หลักสี่ 
    //        จังหวัด กรุงเทพ เมื่อ {strDateTH} 
    //</br>ระหว่าง {result.Contract_Organization} 
    ...
    //         $"สำนักงานใหญ่อยู่เลขที่ {result.ContractorAddressNo} ถนน {result.ContractorStreet} ตำบล/แขวง<br/> {result.ContractorSubDistrict} " +
    //         $"อำเภอ/เขต {result.ContractorDistrict} จังหวัด {result.ContractorProvince}<br/>โดย {result.ContractorSignatoryName} " +
    //         $"มีอำนาจลงนามผูกพันนิติบุคคลปรากฏตามหนังสือรับรองของสำนักงานทะเบียนหุ้นส่วนบริษัท …………… " +
    //         $"ลงวันที่ {CommonDAO.ToThaiDateStringCovert(result.ContractSignDate ?? DateTime.Now)} (5)(และหนังสือมอบอำนาจลง {CommonDAO.ToThaiDateStringCovert(result.ContractSignDate ?? DateTime.Now)}) แนบ<br/>ท้ายสัญญานี้"
    //       : $"(6)(ในกรณีที่ผู้รับจ้างเป็นบุคคลธรรมดาให้ใช้ข้อความว่า กับ {result.ContractorName} " +
    //         $"อยู่บ้านเลขที่ {result.ContractorAddressNo} ถนน {result.ContractorStreet} ตำบล/แขวง <br/>{result.ContractorSubDistrict} " +
    //         $"อำเภอ/เขต {result.ContractorDistrict} จังหวัด {result.ContractorProvince} " +
    ...
    //    </p>
    //    <p class='tab4 t-16'>คู่สัญญาได้ตกลงกันมีข้อความดังต่อไปนี้</p>

    //    <p class='tab3 t-16'><b>ข้อ1 คำนิยาม</b></p>
    //    <p class='tab4 t-16'>
    //        ผู้ซื้อตกลงซื้อและผู้ขายตกลงขายและติดตั้งเครื่องคอมพิวเตอร์ อุปกรณ์การประมวลผล
    //        ระบบคอมพิวเตอร์ 
    //</br>ซึ่งเป็นผลิตภัณฑ์ของ {result.Computer_Model}
    //        ซึ่งต่อไปในสัญญานี้เรียกว่า “คอมพิวเตอร์” ตามรายละเอียดเอกสารแนบท้ายสัญญาผนวก ๑ 
    //</br>รวมเป็นราคาคอมพิวเตอร์และค่าติดตั้งทั้งสิ้น {result.TotalAmount?.ToString("N2") ?? "0.00"} บาท ({CommonDAO.NumberToThaiText(result.TotalAmount ?? 0)})
    //        ซึ่งได้รวมภาษีมูลค่าเพิ่ม จำนวน {result.VatAmount?.ToString("N2") ?? "0.00"} บาท({CommonDAO.NumberToThaiText(result.VatAmount ?? 0)}) ตลอดจนภาษีอากรอื่นๆ และค่าใช้จ่ายทั้งปวงด้วยแล้ว
    //    </p>
    //    <p class='tab4 t-16'>
    //        ในกรณีที่ผู้ขายประสงค์จะนำคอมพิวเตอร์และอุปกรณ์รายการใดแตกต่างไปจากรายละเอียดที่กำหนดไว้ใน
    //</br>เอกสารแนบท้ายสัญญาผนวก๑ มาติดตั้งให้ผู้ซื้อ ผู้ขายจะต้องได้รับความเห็นชอบเป็นหนังสือจากผู้ซื้อก่อน 
    //</br>และคอมพิวเตอร์ที่จะนำมาติดตั้งดังกล่าวนั้นจะต้องมีคุณสมบัติไม่ต่ำกว่าที่กำหนดไว้ในเอกสารแนบท้ายสัญญา
    //</br>ผนวก ๑ และ ๒ ทั้งนี้ จะต้องไม่คิดราคาเพิ่มจากผู้ซื้อไม่ว่าในกรณีใด
    //    </p>
    //    <p class='tab3 t-16'><b>ข้อ ๒ การรับรองคุณภาพ</b></p>
    //    <p class='tab4 t-16'>
    //        ผู้ขายรับรองว่าคอมพิวเตอร์ที่ขายให้ตามสัญญานี้เป็นของแท้ ของใหม่ ไม่ใช่เครื่องที่ใช้งานแล้วนำมาปรับปรุง
    //</br>สภาพขึ้นใหม่และมีคุณสมบัติไม่ต่ำกว่าที่กำหนดไว้ตามรายละเอียด และคุณลักษณะเฉพาะของคอมพิวเตอร์ที่กำหนด
    //</br>ไว้ในเอกสารแนบท้ายสัญญาผนวก ๒
    //    </p>
    //    <p class='tab3 t-16'><b>ข้อ ๓ เอกสารอันเป็นส่วนหนึ่งของสัญญา</b></p>
    //    <p class='tab4 t-16'>
    //        เอกสารแนบท้ายสัญญาดังต่อไปนี้ ให้ถือเป็นส่วนหนึ่งของสัญญานี้<br/></p>
    //        {htmlDocAtt} 
    //    <p class='tab4 t-16'>
    //        ความใดในเอกสารแนบท้ายสัญญาที่ขัดหรือแย้งกับข้อความในสัญญานี้ ให้ใช้ข้อความในสัญญานี้บังคับ และใน
    //</br>กรณีที่เอกสารแนบท้ายสัญญาขัดแย้งกันเอง ผู้ขายจะต้องปฏิบัติตามคำวินิจฉัยของผู้ซื้อ คำวินิจฉัยของผู้ซื้อให้ถือเป็นที่สุด
    //</br>และผู้ขายไม่มีสิทธิเรียกร้องราคา ค่าเสียหาย หรือค่าใช้จ่ายใด ๆ เพิ่มเติมจากผู้ซื้อทั้งสิ้น
    //    </p>
    //    <p class='tab3 t-16'><b>ข้อ ๔ การส่งมอบและติดตั้ง</b></p>
    //    <p class='tab4 t-16'>
    //        ผู้ขายจะส่งมอบและติดตั้งคอมพิวเตอร์ที่ซื้อขายตามสัญญานี้ให้ถูกต้องและครบถ้วนตามที่กำหนดไว้ในข้อ ๑ 
    //</br>ให้พร้อมที่จะใช้งานได้ตามรายละเอียดการทดสอบการใช้งานคอมพิวเตอร์ เอกสารแนบท้ายสัญญาผนวก๓ ให้แก่ผู้ซื้อ ณ {result.DeliveryLocation}
    //</br>และส่งมอบให้แก่ผู้ซื้อภายใน {result.DeliveryDateIn} วัน นับถัดจากวันลงนามในสัญญา ผู้ขายจะต้องแจ้งกำหนดเวลาติดตั้งแล้วเสร็จพร้อมที่
    //</br>จะใช้งานและส่งมอบคอมพิวเตอร์ได้โดยทำเป็นหนังสือยื่นต่อผู้ซื้อ ณ {result.Contract_Sign_Address} 
    //</br>ในวันและเวลาทำการของผู้ซื้อก่อนวันกำหนดส่งมอบไม่น้อยกว่า {result.NotiDaysBeforeDelivery} วันทำการของผู้ซื้อผู้ขายต้องออกแบบสถานที่
    //</br>ติดตั้งคอมพิวเตอร์รวมทั้งระบบอื่นๆ ที่เกี่ยวข้องตามมาตรฐานของผู้ขายและได้รับความเห็นชอบจากผู้ซื้อเป็นหนังสือ และผู้ขายต้องจัดหาเจ้าหน้าที่มาให้คำแนะนำและตรวจสอบความถูกต้องเหมาะสมของสถานที่ให้ทันต่อการติดตั้งคอมพิวเตอร์โดยไม่คิดค่าใช้จ่ายใด ๆ จากผู้ซื้อภายใน {result.LocationPrepareDays} วัน นับถัดจากวันลงนามในสัญญา
    //    </p>
    ...
    //    <p class='tab4 t-16'>
    //        เมื่อผู้ซื้อได้ตรวจรับคอมพิวเตอร์ที่ส่งมอบและติดตั้งแล้วเห็นว่าถูกต้องครบถ้วนตามสัญญานี้แล้ว ผู้ซื้อจะออก
    //</br>หลักฐานการรับมอบไว้เป็นหนังสือ เพื่อผู้ขายนำมาใช้เป็นหลักฐานประกอบการขอรับเงินค่าคอมพิวเตอร์ ตามที่ระบุไว้ใน
    //</br>ข้อ ๖ ถ้าผลของการตรวจรับปรากฏว่า คอมพิวเตอร์ที่ผู้ขายส่งมอบไม่ตรงตามข้อ ๑ หรือมีคุณสมบัติไม่ถูกต้องตาม
    //</br>ข้อ ๒ หรือใช้งานได้ไม่ครบถ้วนตามข้อ ๔ ผู้ซื้อทรงไว้ซึ่งสิทธิที่จะไม่รับคอมพิวเตอร์นั้น ในกรณีเช่นว่านี้ ผู้ขายต้อง
    //</br>รีบนำคอมพิวเตอร์นั้นกลับคืนโดยเร็วที่สุดเท่าที่จะทำได้และนำคอมพิวเตอร์มาส่งมอบให้ใหม่ หรือต้องทำการแก้ไข
    //</br>ให้ถูกต้องตามสัญญาด้วยค่าใช้จ่ายของผู้ขายเอง และระยะเวลาที่เสียไปเพราะเหตุดังกล่าวผู้ขายจะนำมาอ้างเป็นเหตุ
    //</br>ขอขยายเวลาทำการตามสัญญาหรือของดหรือลดค่าปรับไม่ได้  </p>
    // <p class='tab4 t-16'>(8) ในกรณีที่ผู้ขายส่งมอบคอมพิวเตอร์ถูกต้องแต่ไม่ครบจำนวน หรือส่งมอบครบจำนวนแต่ไม่ถูกต้องทั้งหมด 
    //</br>ผู้ซื้อจะตรวจรับเฉพาะส่วนที่ถูกต้อง โดยออกหลักฐานการตรวจรับเฉพาะส่วนนั้นก็ได้ (ความในวรรคสามนี้ จะไม่กำหนด
    //</br>ไว้ในกรณีที่ผู้ซื้อต้องการคอมพิวเตอร์ทั้งหมดในคราวเดียวกัน หรือการซื้อคอมพิวเตอร์ที่ประกอบเป็นชุดหรือหน่วย ถ้าขาดส่วน
    //</br>ประกอบอย่างหนึ่งอย่างใดไปแล้ว จะไม่สามารถใช้งานได้โดยสมบูรณ์)
    //    </p>
    //   <!-- ข้อ ๖ การชำระเงิน -->
    //<p class='tab3 t-16'><b>ข้อ ๖ การชำระเงิน</b></p>
    //<p class='tab4 t-16'>
    //    (9) ผู้ซื้อจะชำระเงินค่าคอมพิวเตอร์ให้แก่ผู้ขาย เมื่อผู้ซื้อได้รับมอบคอมพิวเตอร์ตามข้อ ๕ ไว้โดยครบถ้วนแล้ว</p>
    //   <p class='tab4 t-16'>(10) ผู้ซื้อตกลงชำระเงินค่าคอมพิวเตอร์ให้แก่ผู้ขาย ดังนี้</p>
    // <p class='tab4 t-16'>๖.๑ เงินล่วงหน้า จำนวน {result.AdvancePayment} บาท ({CommonDAO.NumberToThaiText(result.AdvancePayment ?? 0)}) 
    //จะจ่ายให้ภายใน {result.PaymentDueDays} วัน นับถัดจากวันลงนามในสัญญา ทั้งนี้ โดยผู้ขายจะต้องนำหลักประกันการรับเงินล่วงหน้าเป็น {result.PaymentGuaranteeType} {result.PaymentGuaranteeTypeOther}
    //เต็มตามจำนวนเงินล่วงที่ได้รับ มามอบให้แก่ผู้ซื้อเป็นหลักประกันการชำระคืนเงินล่วงหน้าก่อนการรับชำระเงินล่วงหน้านั้น
    //และผู้ซื้อจะคืนหลักประกันการรับเงินล่วงหน้าให้แก่ผู้ขาย เมื่อผู้ซื้อจ่ายเงินที่เหลือตามข้อ ๖.๒ แล้ว เว้นแต่ในกรณีดังต่อไปนี้ ผู้ขายมีสิทธิขอคืนหลักประกันการรับเงินล่วงหน้าบางส่วนก่อนได้</p>
    //  <p class='tab4 t-16'>(๑) กรณีผู้ขายได้วางหลักประกันการรับเงินล่วงหน้าไว้ฉบับเดียว หากผู้ซื้อได้หักเงินล่วงหน้าไปแล้ว 
    //ผู้ขายมีสิทธิขอคืนหลักประกันการรับเงินล่วงหน้าในส่วนที่ผู้ซื้อได้หักเงินล่วงหน้าไปแล้วนั้น
    //โดยผู้ขายจะต้องนำหลักประกันการรับเงินล่วงหน้าฉบับใหม่ที่มีมูลค่าเท่ากับเงินล่วงหน้าที่เหลืออยู่มาวางให้แก่ผู้ซื้อ</p>
    //  <p class='tab4 t-16'>(๒) กรณีผู้ขายได้วางหลักประกันการรับเงินล่วงหน้าไว้หลายฉบับ ซึ่งแต่ละฉบับมีมูลค่าเท่ากับจำนวนเงินล่วงหน้าที่ผู้ซื้อจะต้องหักไว้ในแต่ละงวด
    //หากผู้ซื้อได้หักเงินล่วงหน้าในงวดใดแล้วผู้ขายมีสิทธิขอคืนหลักประกันการรับเงินล่วงหน้าในงวดนั้นได้</p>
    //  <p class='tab4 t-16'>๖.๒ เงินที่เหลือ จำนวน {result.RemainingPaymentAmount} บาท ({CommonDAO.NumberToThaiText(result.RemainingPaymentAmount ?? 0)}) 
    //จะจ่ายให้เมื่อผู้ซื้อได้รับคอมพิวเตอร์ตามข้อ ๕ ไว้โดยถูกต้องครบถ้วน และผู้ขายได้อบรมเจ้าหน้าที่ของผู้ซื้อตามข้อ ๑๐ เสร็จสิ้นแล้ว</p>
    //  <p class='tab4 t-16'>(11) การจ่ายเงินตามเงื่อนไขแห่งสัญญานี้ ผู้ซื้อจะโอนเงินเข้าบัญชีเงินฝากธนาคารของผู้ขาย ชื่อธนาคาร {result.SaleBankName} สาขา {result.SaleBankBranch} 
    //ชื่อบัญชี {result.SaleBankAccountName} เลขที่บัญชี {result.SaleBankAccountNumber} ทั้งนี้ ผู้ขายตกลงเป็นผู้รับภาระเงินค่าธรรมเนียม หรือค่าบริการอื่นใดเกี่ยวกับการโอน 
    //รวมทั้งค่าใช้จ่ายอื่นใด (ถ้ามี) ที่ธนาคารเรียกเก็บและยินยอมให้มีการหักเงินดังกล่าวจากจำนวนเงินโอนในงวดนั้น ๆ
    //</p>

    //<!-- ข้อ ๗ การรับประกันความชำรุดบกพร่อง -->
    //<p class='tab3 t-16'><b>ข้อ ๗ การรับประกันความชำรุดบกพร่อง</b></p>
    //<p class='tab4 t-16'>
    ...
    //นับถัดจากวันที่ผู้ซื้อได้รับมอบคอมพิวเตอร์ทั้งหมดโดยถูกต้องครบถ้วนตามสัญญา ถ้าภายในระยะเวลาดังกล่าวคอมพิวเตอร์ชำรุดบกพร่องหรือขัดข้อง 
    //หรือใช้งานไม่ได้ทั้งหมดหรือแต่บางส่วน หรือเกิดความชำรุดบกพร่องหรือขัดข้องจากการติดตั้ง เว้นแต่ความชำรุดบกพร่องหรือขัดข้องดังกล่าว 
    //เกิดขึ้นจากความผิดของผู้ซื้อซึ่งไม่ได้เกิดขึ้นจากการใช้งานตามปกติ ผู้ขายจะต้องจัดการซ่อมแซมแก้ไขให้อยู่ในสภาพใช้การได้ดีดังเดิม 
    //โดยต้องเริ่มจัดการซ่อมแซมแก้ไขภายใน {result.DaysToRepairAfterNoti} วัน นับถัดจากวันที่ได้รับแจ้งจากผู้ซื้อโดยไม่คิดค่าใช้จ่ายใด ๆ 
    //จากผู้ซื้อทั้งสิ้น ถ้าผู้ขายไม่จัดการซ่อมแซมแก้ไขภายในกำหนดเวลาดังกล่าว ผู้ซื้อมีสิทธิที่จะทำการนั้นเองหรือจ้างผู้อื่นทำการนั้นแทนผู้ขาย โดยผู้ขายต้องออกค่าใช้จ่ายเองทั้งสิ้นแทนผู้ซื้อ</p>
    //<p class='tab4 t-16'>
    ...
    //โดยให้มีเวลาคอมพิวเตอร์ขัดข้องรวมตามเกณฑ์การคำนวณเวลาขัดข้องไม่เกินเดือนละ {result.MaximumDownTimeHours} ชั่วโมง หรือร้อยละ {result.MaximumDownTimePercent}(%) 
    //ของเวลาใช้งานทั้งหมดของคอมพิวเตอร์ของเดือนนั้น แล้วแต่ตัวเลขใดจะมากกว่ากัน มิฉะนั้นผู้ขายต้องยอมให้ผู้ซื้อคิดค่าปรับเป็นรายชั่วโมง ในอัตราร้อยละ {result.PenaltyPerHourPercent} 
    //ของราคาคอมพิวเตอร์ทั้งหมดตามสัญญานี้คิดเป็นเงิน {result.PenaltyPerHour} บาท ({CommonDAO.NumberToThaiText(result.PenaltyPerHour ?? 0)}) ต่อชั่วโมง 
    //ในช่วงเวลาที่ไม่สามารถใช้คอมพิวเตอร์ได้ในส่วนที่เกินกว่ากำหนดเวลาขัดข้องข้างต้น</p>
    //   <p class='tab4 t-16'> เกณฑ์การคำนวณเวลาขัดข้องของคอมพิวเตอร์ตามวรรคสอง ให้เป็นดังนี้</p>
    //   <p class='tab4 t-16'> - กรณีที่คอมพิวเตอร์เกิดขัดข้องพร้อมกันหลายหน่วย ให้นับเวลาขัดข้องของหน่วยที่มีตัวถ่วงมากที่สุดเพียงหน่วยเดียว</p>
    //   <p class='tab4 t-16'> - กรณีความเสียหายอันสืบเนื่องมาจากความขัดข้องของคอมพิวเตอร์แตกต่างกัน เวลาที่ใช้ในการคำนวณค่าปรับจะเท่ากับเวลาขัดข้องของคอมพิวเตอร์หน่วยนั้นคูณด้วยตัวถ่วงซึ่งมีค่าต่างๆ ตามเอกสารแนบท้ายสัญญาผนวก ๔</p>
    //   <p class='tab4 t-16'> การที่ผู้ซื้อทำการนั้นเอง หรือให้ผู้อื่นทำการนั้นแทนผู้ขาย ไม่ทำให้ผู้ขายหลุดพ้นจาก ความรับผิดตามสัญญา หากผู้ขายไม่ชดใช้ค่าใช้จ่ายหรือค่าเสียหายตามที่ผู้ซื้อเรียกร้องผู้ซื้อมีสิทธิบังคับจากหลักประกันการปฏิบัติตามสัญญาได้
    //</p>

    //<!-- ข้อ ๘ หลักประกันการปฏิบัติตามสัญญา -->
    //<p class='tab3 t-16'><b>ข้อ ๘ หลักประกันการปฏิบัติตามสัญญา</b></p>
    //<p class='tab4 t-16'>
    //    ในขณะทำสัญญานี้ผู้ขายได้นำหลักประกันเป็น {result.PerformanceGuarantee} เป็นจำนวนเงิน {result.GuaranteeAmount} บาท ({CommonDAO.NumberToThaiText(result.GuaranteeAmount ?? 0)}) ซึ่งเท่ากับร้อยละ {result.GuaranteePercent} ของราคาซื้อขายคอมพิวเตอร์ตามข้อ ๑ มามอบให้แก่ผู้ซื้อเพื่อเป็นหลักประกันการปฏิบัติตามสัญญานี้</p>
    //    <p class='tab4 t-16'>(16) กรณีผู้ขายใช้หนังสือค้ำประกันมาเป็นหลักประกันการปฏิบัติตามสัญญา หนังสือค้ำประกันดังกล่าวจะต้องออกโดยธนาคารที่ประกอบกิจการในประเทศไทย
    //หรือโดยบริษัทเงินทุนหรือบริษัทเงินทุนหลักทรัพย์ที่ได้รับอนุญาตให้ประกอบกิจการเงินทุนเพื่อการพาณิชย์และประกอบธุรกิจค้ำประกันตามประกาศของธนาคารแห่งประเทศไทย
    //ตามรายชื่อบริษัทเงินทุนที่ธนาคารแห่งประเทศไทยแจ้งเวียนให้ทราบตามแบบที่คณะกรรมการนโยบายการจัดซื้อจัดจ้างและการบริหารพัสดุภาครัฐกำหนด
    //หรืออาจเป็นหนังสือค้ำประกันอิเล็กทรอนิกส์ตามวิธีการที่กรมบัญชีกลางกำหนดก็ได้และจะต้องมีอายุการค้ำประกันตลอดไปจนกว่าผู้ขายพ้นข้อผูกพันตามสัญญานี้</p>
    //   <p class='tab4 t-16'> หลักประกันที่ผู้ขายนำมามอบให้ตามวรรคหนึ่ง จะต้องมีอายุครอบคลุมความรับผิดทั้งปวงของผู้ขายตลอดอายุสัญญา
    //ถ้าหลักประกันที่ผู้ขายนำมามอบให้ดังกล่าวลดลงหรือเสื่อมค่าลง หรือมีอายุไม่ครอบคลุมถึงความรับผิดของผู้ขายตลอดอายุสัญญา </p>
    // <p class='tab4 t-16'> ม่ว่าด้วยเหตุใดๆ ก็ตาม รวมถึงกรณีผู้ขายส่งมอบและติดตั้งคอมพิวเตอร์ล่าช้าเป็นเหตุให้ระยะเวลาแล้วเสร็จหรือวันครบกำหนดความรับผิดในความชำรุดบกพร่องตามสัญญาเปลี่ยนแปลงไป 
    //ไม่ว่าจะเกิดขึ้นคราวใด ผู้ขายต้องหาหลักประกันใหม่หรือหลักประกันเพิ่มเติมให้มีจำนวนครบถ้วนตามวรรคหนึ่ง มามอบให้แก่ผู้ซื้อภายใน {result.NewGuaranteeDays} วัน นับถัดจากวันที่ได้รับแจ้งเป็นหนังสือจากผู้ซื้อ</p>
    //<p class='tab4 t-16'>หลักประกันที่ผู้ขายนำมามอบไว้ตามข้อนี้ ผู้ซื้อจะคืนให้แก่ผู้ขายโดยไม่มีดอกเบี้ยเมื่อผู้ขายพ้นจากข้อผูกพันและความรับผิดทั้งปวงตามสัญญานี้แล้ว
    //</p>

    //<!-- ข้อ ๙ การโอนกรรมสิทธิ์ -->
    //<p class='tab3 t-16'><b>ข้อ ๙ การโอนกรรมสิทธิ์</b></p>
    //<p class='tab4 t-16'>
    //    คู่สัญญาตกลงกันว่ากรรมสิทธิ์ในคอมพิวเตอร์ตามสัญญาจะโอนไปยังผู้ซื้อเมื่อผู้ซื้อได้รับมอบคอมพิวเตอร์ดังกล่าวตามข้อ ๕ แล้ว
    //</p>

    //<!-- ข้อ ๑๐ การอบรม -->
    //<p class='tab3 t-16'><b>ข้อ ๑๐ การอบรม</b></p>
    //<p class='tab4 t-16'>
    ...
    //<p class='tab3 t-16'><b>ข้อ ๑๑ คู่มือการใช้คอมพิวเตอร์</b></p>
    //<p class='tab4 t-16'>
    ...
    //<p class='tab3 t-16'><b>ข้อ ๑๒ การรับประกันความเสียหาย</b></p>
    //<p class='tab4 t-16'>
    ...
    //<p class='tab3 t-16'><b>ข้อ ๑๓ การบอกเลิกสัญญา</b></p>
    //<p class='tab4 t-16'>
    //    เมื่อครบกำหนดส่งมอบคอมพิวเตอร์ตามสัญญาแล้ว หากผู้ขายไม่ส่งมอบและติดตั้งคอมพิวเตอร์บางรายการหรือทั้งหมดให้แก่ผู้ซื้อภายในกำหนดเวลาดังกล่าว หรือส่งมอบคอมพิวเตอร์ไม่ตรงตามสัญญาหรือมีคุณสมบัติไม่ถูกต้องตามสัญญา 
    //หรือส่งมอบและติดตั้งแล้วเสร็จภายในกำหนดแต่ไม่สามารถใช้งานได้อย่างมีประสิทธิภาพ หรือใช้งานไม่ได้ครบถ้วนตามสัญญา หรือผู้ขายไม่ปฏิบัติตามสัญญาข้อใดข้อหนึ่ง 
    //ผู้ซื้อมีสิทธิบอกเลิกสัญญาทั้งหมดหรือแต่บางส่วนได้ การใช้สิทธิบอกเลิกสัญญานั้นไม่กระทบถึงสิทธิของผู้ซื้อที่จะเรียกร้องค่าเสียหายจากผู้ขาย</p>
    // <p class='tab4 t-16'>   ในกรณีที่ผู้ซื้อใช้สิทธิบอกเลิกสัญญา ผู้ซื้อมีสิทธิริบหรือบังคับจากหลักประกันตาม (17) (ข้อ ๖ และ) ข้อ ๘ เป็นจำนวนเงินทั้งหมดหรือแต่บางส่วนก็ได้ แล้วแต่ผู้ซื้อจะเห็นสมควร 
    //และถ้าผู้ซื้อจัดซื้อคอมพิวเตอร์รวมถึงการติดตั้งจากบุคคลอื่นเต็มจำนวนหรือเฉพาะจำนวนที่ขาดส่ง แล้วแต่กรณี ภายในกำหนด {result.TeminationNewMonths} เดือน 
    //นับถัดจากวันบอกเลิกสัญญา ผู้ขายจะต้องชดใช้ราคาที่เพิ่มขึ้นจากราคาที่กำหนดไว้ในสัญญานี้ด้วย รวมทั้งค่าใช้จ่ายใด ๆ ที่ผู้ซื้อต้องใช้จ่ายในการจัดหาผู้ขายรายใหม่ดังกล่าวด้วย</p>

    // <p class='tab4 t-16'> ในกรณีที่ผู้ขายได้ส่งมอบคอมพิวเตอร์ให้แก่ผู้ซื้อและผู้ซื้อบอกเลิกสัญญา ผู้ขายจะต้องนำคอมพิวเตอร์กลับคืนไป และทำสถานที่ที่รื้อถอนคอมพิวเตอร์ออกไปให้มีสภาพดังที่มีอยู่เดิมก่อนทำสัญญานี้ภายใน {result.ReturnDaysIn} วัน 
    //นับถัดจากวันที่ผู้ซื้อบอกเลิกสัญญา โดยผู้ขายเป็นผู้เสียค่าใช้จ่ายเองทั้งสิ้น</p>

    // <p class='tab4 t-16'> (19)ถ้าผู้ขายไม่ยอมนำคอมพิวเตอร์กลับคืนไปภายในกำหนดเวลาดังกล่าวตามวรรคสาม ผู้ซื้อจะกำหนดเวลาให้ผู้ขายนำคอมพิวเตอร์กลับคืนไปอีกครั้งหนึ่ง 
    //หากพ้นกำหนดเวลาดังกล่าวแล้ว ผู้ขายยังไม่นำคอมพิวเตอร์กลับคืนไปอีก ผู้ซื้อมีสิทธินำคอมพิวเตอร์ออกขายทอดตลาด เงินที่ได้จากการขายทอดตลาด ผู้ขายยอมให้ผู้ซื้อหักเป็นค่าปรับและหักเป็นค่าใช้จ่าย 
    //และค่าเสียหายที่เกิดแก่ผู้ซื้อ ซึ่งรวมถึงค่าใช้จ่ายต่างๆ ที่ผู้ซื้อได้เสียไปในการดำเนินการขายทอดตลาดคอมพิวเตอร์ดังกล่าว ค่าใช้จ่ายในการทำสถานที่ที่รื้อถอนคอมพิวเตอร์ออกไปให้มีสภาพดังที่มีอยู่เดิมก่อนทำสัญญานี้ 
    //เงินที่เหลือจากการหักค่าปรับ ค่าใช้จ่าย และค่าเสียหายดังกล่าวแล้วผู้ซื้อจะคืนให้แก่ผู้ขาย</p>
    //    <p class='tab4 t-16'>  เมื่อผู้ซื้อบอกเลิกสัญญาแล้ว ผู้ซื้อไม่ต้องรับผิดชอบในความเสียหายใด ๆ ทั้งสิ้นอันเกิดแก่คอมพิวเตอร์ซึ่งอยู่ในความครอบครองของผู้ซื้อ
    //</p>

    //<!-- ข้อ ๑๔ ค่าปรับ -->
    //<p class='tab3 t-16'><b>ข้อ ๑๔ ค่าปรับ</b></p>
    //<p class='tab4 t-16'>
    //    ในกรณีที่ผู้ซื้อยังไม่ใช้สิทธิบอกเลิกสัญญาตามข้อ ๑๓ ผู้ขายจะต้องชำระค่าปรับให้ผู้ซื้อเป็นรายวัน ในอัตราร้อยละ {result.FinePerDaysPercent} ของราคาคอมพิวเตอร์ที่ยังไม่ได้รับมอบ 
    //นับถัดจากวันครบกำหนดตามสัญญาจนถึงวันที่ผู้ขายได้นำคอมพิวเตอร์มาส่งมอบและติดตั้งให้แก่ผู้ซื้อจนถูกต้องครบถ้วนตามสัญญา</p>
    //  <p class='tab4 t-16'>  การคิดค่าปรับในกรณีที่คอมพิวเตอร์ที่ตกลงซื้อขายเป็นระบบ ถ้าผู้ขายส่งมอบเพียงบางส่วนหรือขาดส่วนประกอบส่วนหนึ่งส่วนใดไป 
    //หรือส่งมอบและติดตั้งทั้งหมดแล้วแต่ใช้งานไม่ได้ถูกต้องครบถ้วน ให้ถือว่ายังไม่ได้ส่งมอบคอมพิวเตอร์นั้นเลย และคิดค่าปรับจากราคาคอมพิวเตอร์ทั้งระบบ</p>
    //    <p class='tab4 t-16'> ในระหว่างที่ผู้ซื้อยังไม่ได้ใช้สิทธิบอกเลิกสัญญานั้น หากผู้ซื้อเห็นว่าผู้ขายไม่อาจปฏิบัติตามสัญญาต่อไปได้ ผู้ซื้อจะใช้สิทธิบอกเลิกสัญญา 
    //และริบหรือบังคับจากหลักประกันตาม (21) (ข้อ ๖ และ) ข้อ ๘ กับเรียกร้องให้ชดใช้ราคาที่เพิ่มขึ้นตามที่กำหนดไว้ในข้อ ๑๓ วรรคสอง ก็ได้ 
    ...
    //<p class='tab3 t-16'><b>ข้อ ๑๕ การบังคับค่าปรับ ค่าเสียหาย และค่าใช้จ่าย</b></p>
    //<p class='tab4 t-16'>
    //    ในกรณีที่ผู้ขายไม่ปฏิบัติตามสัญญาข้อใดข้อหนึ่งด้วยเหตุใด ๆ ก็ตาม จนเป็นเหตุให้เกิดค่าปรับ ค่าเสียหาย หรือค่าใช้จ่ายแก่ผู้ซื้อ ผู้ขายต้องชดใช้ค่าปรับ ค่าเสียหาย หรือค่าใช้จ่ายดังกล่าวให้แก่ผู้ซื้อ 
    //โดยสิ้นเชิงภายในกำหนด {result.EnforcementOfFineDays} วัน นับถัดจากวันที่ได้รับแจ้งเป็นหนังสือจากผู้ซื้อ 
    //หากผู้ขายไม่ชดใช้ให้ถูกต้องครบถ้วนภายในระยะเวลาดังกล่าวให้ผู้ซื้อมีสิทธิที่จะหักเอาจากค่าคอมพิวเตอร์ที่ต้องชำระ หรือบังคับจากหลักประกันการปฏิบัติตามสัญญาได้ทันที<p/>
    //     <p class='tab4 t-16'> หากค่าปรับ ค่าเสียหาย หรือค่าใช้จ่ายที่บังคับจากค่าคอมพิวเตอร์ที่ต้องชำระหรือหลักประกันการปฏิบัติตามสัญญาแล้วยังไม่เพียงพอ 
    //ผู้ขายยินยอมชำระส่วนที่เหลือที่ยังขาดอยู่จนครบถ้วนตามจำนวนค่าปรับ ค่าเสียหาย หรือค่าใช้จ่ายนั้น ภายในกำหนด {result.OutstandingPeriodDays} วัน นับถัดจากวันที่ได้รับแจ้งเป็นหนังสือจากผู้ซื้อ<p/>
    //     <p class='tab4 t-16'> หากมีเงินค่าคอมพิวเตอร์ที่ซื้อขายตามสัญญาที่หักไว้จ่ายเป็นค่าปรับ ค่าเสียหาย หรือค่าใช้จ่ายแล้วยังเหลืออยู่อีกเท่าใด ผู้ซื้อจะคืนให้แก่ผู้ขายทั้งหมด
    //</p>

    //<!-- ข้อ ๑๖ การงดหรือลดค่าปรับ หรือขยายเวลาในการปฏิบัติตามสัญญา -->
    //<p class='tab3 t-16'><b>ข้อ ๑๖ การงดหรือลดค่าปรับ หรือขยายเวลาในการปฏิบัติตามสัญญา</b></p>
    //<p class='tab4 t-16'>
    ...
    //ซึ่งออกตามความในกฎหมายว่าด้วยการจัดซื้อจัดจ้างและการบริหารพัสดุภาครัฐ ทำให้ผู้ขายไม่สามารถส่งมอบและติดตั้งคอมพิวเตอร์ หรือบำรุงรักษาและซ่อมแซมแก้ไขคอมพิวเตอร์
    //ตามเงื่อนไขและกำหนดเวลาแห่งสัญญานี้ได้ ผู้ขายมีสิทธิของดหรือลดค่าปรับหรือขยายเวลาทำการตามสัญญา โดยจะต้องแจ้งเหตุหรือพฤติการณ์ดังกล่าวพร้อมหลักฐานเป็นหนังสือให้ผู้ซื้อทราบภายใน ๑๕ (สิบห้า) วัน 
    //นับถัดจากวันที่เหตุนั้นสิ้นสุดลง หรือตามที่กำหนดในกฎกระทรวงดังกล่าว<p/>
    //    <p class='tab4 t-16'>ถ้าผู้ขายไม่ปฏิบัติให้เป็นไปตามความในวรรคหนึ่ง ให้ถือว่าผู้ขายได้สละสิทธิเรียกร้องในการที่จะของดหรือลดค่าปรับหรือขยายเวลาทำการตามสัญญา 
    //โดยไม่มีเงื่อนไขใด ๆ ทั้งสิ้น เว้นแต่กรณีเหตุเกิดจากความผิดหรือความบกพร่องของฝ่ายผู้ซื้อซึ่งมีหลักฐานชัดแจ้งหรือผู้ซื้อทราบดีอยู่แล้วตั้งแต่ต้น<p/>
    //   <p class='tab4 t-16'> การงดหรือลดค่าปรับหรือขยายเวลาทำการตามสัญญาตามวรรคหนึ่ง อยู่ในดุลพินิจของผู้ซื้อที่จะพิจารณาตามที่เห็นสมควร
    //</p>

    //<!-- ข้อ ๑๗ การใช้เรือไทย -->
    //<p class='tab3 t-16'><b>ข้อ ๑๗ การใช้เรือไทย</b></p>
    //<p class='tab4 t-16'>
    //    ถ้าผู้ขายจะต้องสั่งหรือนำเข้าคอมพิวเตอร์มาจากต่างประเทศและต้องนำเข้ามาโดยทางเรือในเส้นทางเดินเรือที่มีเรือไทยเดินอยู่
    //และสามารถให้บริการรับขนได้ตามที่รัฐมนตรีว่าการกระทรวงคมนาคมประกาศกำหนด ผู้ขายต้องจัดการให้คอมพิวเตอร์บรรทุกโดยเรือไทยหรือเรือที่มีสิทธิเช่นเดียวกับเรือไทยจากต่างประเทศมายังประเทศไทย 
    //เว้นแต่จะได้รับอนุญาตจากกรมเจ้าท่าก่อนบรรทุกคอมพิวเตอร์ลงเรืออื่นที่มิใช่เรือไทยหรือเป็นของที่รัฐมนตรีว่าการกระทรวงคมนาคมประกาศยกเว้นให้บรรทุกโดยเรืออื่นได้ ทั้งนี้ ไม่ว่าการสั่งหรือนำเข้าคอมพิวเตอร์จากต่างประเทศจะเป็นแบบใด<p/>
    //    <p class='tab4 t-16'>ในการส่งมอบคอมพิวเตอร์ให้แก่ผู้ซื้อ ถ้าเป็นกรณีตามวรรคหนึ่งผู้ขายจะต้องส่งมอบใบตราส่ง (Bill of Lading) หรือสำเนาใบตราส่งสำหรับคอมพิวเตอร์นั้น
    //ซึ่งแสดงว่าได้บรรทุกมาโดยเรือไทย หรือเรือที่มีสิทธิเช่นเดียวกับเรือไทยให้แก่ผู้ซื้อพร้อมกับการส่งมอบคอมพิวเตอร์ด้วย<p/>
    //    <p class='tab4 t-16'>ในกรณีที่คอมพิวเตอร์ไม่ได้บรรทุกจากต่างประเทศมายังประเทศไทยโดยเรือไทยหรือเรือที่มีสิทธิเช่นเดียวกับเรือไทย 
    //ผู้ขายต้องส่งมอบหลักฐานซึ่งแสดงว่าได้รับอนุญาตจากกรมเจ้าท่าให้บรรทุกของโดยเรืออื่นได้ หรือหลักฐานซึ่งแสดงว่าได้ชำระค่าธรรมเนียมพิเศษเนื่องจากการไม่บรรทุกของโดยเรือ
    //ไทยตามกฎหมายว่าด้วยการส่งเสริมการพาณิชยนาวีแล้วอย่างใดอย่างหนึ่งแก่ผู้ซื้อด้วย<p/>
    //    <p class='tab4 t-16'>ในกรณีที่ผู้ขายไม่ส่งมอบหลักฐานอย่างใดอย่างหนึ่งดังกล่าวในวรรคสองและวรรคสามให้แก่ผู้ซื้อแต่จะขอส่งมอบคอมพิวเตอร์ให้ผู้ซื้อก่อนโดยยังไม่รับชำระเงินค่าคอมพิวเตอร์
    //ผู้ซื้อมีสิทธิรับคอมพิวเตอร์ไว้ก่อนและชำระเงินค่าคอมพิวเตอร์เมื่อผู้ขายได้ปฏิบัติถูกต้องครบถ้วนดังกล่าวแล้วได้<p/>
    //    <p class='tab4 t-16'>สัญญานี้ทำขึ้นสองฉบับ มีข้อความถูกต้องตรงกัน คู่สัญญาได้อ่านและเข้าใจข้อความโดยละเอียดตลอดแล้ว จึงได้ลงลายมือชื่อพร้อมทั้งประทับตรา (ถ้ามี) ไว้เป็นสำคัญต่อหน้าพยานและคู่สัญญาต่างยึดถือไว้ฝ่ายละหนึ่งฉบับ
    //</p>
    //</br>
    //</br>
    //<!-- 🔹 รายชื่อผู้ลงนาม -->
    ...
    //    (15) ให้กำหนดจำนวนเงินหลักประกันการปฏิบัติตามสัญญาตามระเบียบกระทรวงการคลังว่าด้วยการจัดซื้อจัดจ้าง และการบริหารพัสดุภาครัฐ พ.ศ. ๒๕๖๐ ข้อ ๑๖๘ ทั้งนี้ จะต้องประกาศให้ทราบในเอกสารประกวดราคาด้วย<br/>
    ...
    //    (18) กำหนดเวลาที่ผู้ซื้อจะซื้อสิ่งของจากแหล่งอื่นเมื่อบอกเลิกสัญญาและมีสิทธิเรียกเงินในส่วนที่เพิ่มขึ้น จากราคาที่กำหนดไว้ในสัญญานั้น ให้อยู่ในดุลพินิจของผู้ซื้อโดยตกลงกับผู้ขาย<br/>
    //    (19) ความในวรรคนี้อาจแก้ไขเปลี่ยนแปลงได้ตามความเหมาะสมถ้าหน่วยงานของรัฐผู้ทำสัญญา สามารถกำหนดมาตรการอื่นใดในสัญญา หรือกำหนดทางปฏิบัติเพื่อแก้ปัญหาที่ผู้ขายไม่ยอมนำคอมพิวเตอร์กลับคืนไปได้<br/>
    ...
    //        .tab3 {{ text-indent: 96px;  word-break: break-all; }}
    //        .tab4 {{ text-indent: 144px;  word-break: break-all; }}
...
<br>
    <p class='text-right t-18'>สัญญาเลขที่ {(result.CPAContractNumber ?? "xxxxxx")}</p>
    <p class='tab4 t-16'>
        สัญญาฉบับนี้ทำขึ้น ณ สำนักงานส่งเสริมวิสาหกิจขนาดกลางและขนาดย่อม (สสว.) ตำบล/แขวง ทุ่งสองห้อง อำเภอ/เขต หลักสี่ จังหวัด กรุงเทพ เมื่อ {strDateTH} ระหว่าง {result.Contract_Organization} โดย {result.SignatoryName} ซึ่งต่อไปในสัญญานี้เรียกว่า “ผู้ซื้อ” ฝ่ายหนึ่ง กับ {result.ContractorName}
    </p>
    <p class='tab4 t-16'>
        {(result.ContractorType == "นิติบุคคล"
            ? $"ซึ่งจดทะเบียนเป็นนิติบุคคล ณ {result.ContractorName} มีสำนักงานใหญ่อยู่เลขที่ {result.ContractorAddressNo} ถนน {result.ContractorStreet} ตำบล/แขวง {result.ContractorSubDistrict} อำเภอ/เขต {result.ContractorDistrict} จังหวัด {result.ContractorProvince} โดย {result.ContractorSignatoryName} มีอำนาจลงนามผูกพันนิติบุคคลปรากฏ<br>ตามหนังสือรับรองของสำนักงานทะเบียนหุ้นส่วนบริษัท …………… ลง {CommonDAO.ToThaiDateStringCovert(result.ContractSignDate ?? DateTime.Now)} (5)(และหนังสือมอบอำนาจลง {CommonDAO.ToThaiDateStringCovert(result.ContractSignDate ?? DateTime.Now)}) แนบท้ายสัญญานี้"
            : $"(6)(กับ {result.ContractorName} อยู่บ้านเลขที่ {result.ContractorAddressNo} ถนน {result.ContractorStreet} ตำบล/แขวง {result.ContractorSubDistrict} อำเภอ/เขต {result.ContractorDistrict} จังหวัด {result.ContractorProvince} ผู้ถือบัตรประจำตัวประชาชนเลขที่ {result.CitizenId} ดังปรากฏตามสำเนาบัตรประจำตัวประชาชนแนบท้ายสัญญานี้) ซึ่งต่อไปในสัญญานี้เรียกว่า “ผู้ขาย” อีกฝ่ายหนึ่ง")}
    </p>
    <p class='tab4 t-16'>คู่สัญญาได้ตกลงกันมีข้อความดังต่อไปนี้</p>
    <p class='tab3 t-16'><b>ข้อ ๑ ข้อตกลงซื้อขาย</b></p>
    <p class='tab4 t-16'>
        ผู้ซื้อตกลงซื้อและผู้ขายตกลงขายและติดตั้งเครื่องคอมพิวเตอร์ อุปกรณ์การประมวลผล ระบบคอมพิวเตอร์ ซึ่งเป็นผลิตภัณฑ์ของ {result.Computer_Model} ซึ่งต่อไปในสัญญานี้เรียกว่า “คอมพิวเตอร์” ตามรายละเอียดเอกสารแนบท้ายสัญญาผนวก ๑ รวมเป็นราคาคอมพิวเตอร์และค่าติดตั้งทั้งสิ้น {result.TotalAmount?.ToString("N2") ?? "0.00"} บาท ({CommonDAO.NumberToThaiText(result.TotalAmount ?? 0)}) ซึ่งได้รวมภาษีมูลค่าเพิ่ม จำนวน {result.VatAmount?.ToString("N2") ?? "0.00"} บาท({CommonDAO.NumberToThaiText(result.VatAmount ?? 0)}) ตลอดจนภาษีอากรอื่นๆ และค่าใช้จ่ายทั้งปวงด้วยแล้ว
    </p>
    <p class='tab4 t-16'>
        ในกรณีที่ผู้ขายประสงค์จะนำคอมพิวเตอร์และอุปกรณ์รายการใดแตกต่างไปจากรายละเอียดที่กำหนดไว้ใน<br>
เอกสารแนบท้ายสัญญาผนวก๑ มาติดตั้งให้ผู้ซื้อ ผู้ขายจะต้องได้รับความเห็นชอบเป็นหนังสือจากผู้ซื้อก่อน และคอมพิวเตอร์<br>
ที่จะนำมาติดตั้งดังกล่าวนั้นจะต้องมีคุณสมบัติไม่ต่ำกว่าที่กำหนดไว้ในเอกสารแนบท้ายสัญญาผนวก ๑ และ ๒ ทั้งนี้ จะต้องไม่คิดราคาเพิ่มจากผู้ซื้อไม่ว่าในกรณีใด
    </p>
    <p class='tab3 t-16'><b>ข้อ ๒ การรับรองคุณภาพ</b></p>
    <p class='tab4 t-16'>
        ผู้ขายรับรองว่าคอมพิวเตอร์ที่ขายให้ตามสัญญานี้เป็นของแท้ ของใหม่ ไม่ใช่เครื่อง<br>
ที่ใช้งานแล้วนำมาปรับปรุงสภาพขึ้นใหม่และมีคุณสมบัติไม่ต่ำกว่าที่กำหนดไว้ตามรายละเอียด และคุณลักษณะ<br>เฉพาะของคอมพิวเตอร์ที่กำหนดไว้ในเอกสารแนบท้ายสัญญาผนวก ๒
    </p>
    <p class='tab3 t-16'><b>ข้อ ๓ เอกสารอันเป็นส่วนหนึ่งของสัญญา</b></p>
    <p class='tab4 t-16'>
    ...
    {htmlDocAtt}
    <p class='tab4 t-16'>
        ความใดในเอกสารแนบท้ายสัญญาที่ขัดหรือแย้งกับข้อความในสัญญานี้ ให้ใช้ข้อความในสัญญานี้บังคับ และในกรณีที่เอกสารแนบท้ายสัญญาขัดแย้งกันเอง ผู้ขายจะต้องปฏิบัติตามคำวินิจฉัยของผู้ซื้อ คำวินิจฉัยของผู้ซื้อให้ถือ</br>เป็นที่สุด และผู้ขายไม่มีสิทธิเรียกร้องราคา ค่าเสียหาย หรือค่าใช้จ่ายใด ๆ เพิ่มเติมจากผู้ซื้อทั้งสิ้น
    </p>
    <p class='tab3 t-16'><b>ข้อ ๔ การส่งมอบและติดตั้ง</b></p>
    <p class='tab4 t-16'>
        ผู้ขายจะส่งมอบและติดตั้งคอมพิวเตอร์ที่ซื้อขายตามสัญญานี้ให้ถูกต้องและครบถ้วนตามที่กำหนดไว้ใน
ข้อ ๑ ให้พร้อมที่จะใช้งานได้ตามรายละเอียดการทดสอบการใช้งานคอมพิวเตอร์ เอกสารแนบท้ายสัญญาผนวก๓ ให้แก่ผู้ซื้อ ณ {result.DeliveryLocation} และส่งมอบให้แก่
</br>ผู้ซื้อภายใน {result.DeliveryDateIn} วัน นับถัดจากวันลงนามในสัญญา</p>
<p class='tab4 t-16'>ผู้ขายจะต้องแจ้งกำหนดเวลาติดตั้งแล้วเสร็จพร้อมที่จะใช้งานและส่งมอบ คอมพิวเตอร์ได้</br>โดยทำเป็นหนังสือยื่นต่อผู้ซื้อ ณ {result.Contract_Sign_Address} ในวันและเวลาทำการของผู้ซื้อก่อนวันกำหนดส่งมอบไม่น้อยกว่า {result.NotiDaysBeforeDelivery} วัน ทำการของผู้ซื้อ</p>
<p class='tab4 t-16'>ผู้ขายต้องออกแบบสถานที่ติดตั้งคอมพิวเตอร์รวมทั้งระบบอื่นๆ ที่เกี่ยวข้องตามมาตรฐานของผู้ขาย</br>
และได้รับความเห็นชอบจากผู้ซื้อเป็นหนังสือ และผู้ขายต้องจัดหาเจ้าหน้าที่มาให้คำแนะนำและตรวจสอบความถูกต้อง</br>
เหมาะสมของสถานที่ให้ทันต่อการติดตั้งคอมพิวเตอร์โดยไม่คิดค่าใช้จ่ายใดๆ จากผู้ซื้อภายใน {result.LocationPrepareDays} วัน </br>นับถัดจากวันลงนามในสัญญา
    </p>
    <p class='tab3 t-16'><b>ข้อ ๕ การตรวจรับ</b></p>
    <p class='tab4 t-16'>
        เมื่อผู้ซื้อได้ตรวจรับคอมพิวเตอร์ที่ส่งมอบและติดตั้งแล้วเห็นว่าถูกต้องครบถ้วนตามสัญญานี้แล้ว ผู้ซื้อ</br>
จะออกหลักฐานการรับมอบไว้เป็นหนังสือ เพื่อผู้ขายนำมาใช้เป็นหลักฐานประกอบการขอรับเงินค่าคอมพิวเตอร์ ตามที่
</br>ระบุไว้ในข้อ ๖ </p>
<p class='tab4 t-16'>ถ้าผลของการตรวจรับปรากฏว่า คอมพิวเตอร์ที่ผู้ขายส่งมอบไม่ตรงตามข้อ ๑ หรือมีคุณสมบัติไม่ถูกต้อง
</br>ตามข้อ ๒ หรือใช้งานได้ไม่ครบถ้วนตามข้อ ๔ ผู้ซื้อทรงไว้ซึ่งสิทธิที่จะไม่รับคอมพิวเตอร์นั้น ในกรณีเช่นว่านี้ ผู้ขายต้องรีบนำ
</br>คอมพิวเตอร์นั้นกลับคืนโดยเร็วที่สุดเท่าที่จะทำได้และนำคอมพิวเตอร์มาส่งมอบให้ใหม่ หรือต้องทำการแก้ไขให้ถูกต้อง
</br>ตามสัญญาด้วยค่าใช้จ่ายของผู้ขายเอง และระยะเวลาที่เสียไปเพราะเหตุดังกล่าวผู้ขายจะนำมาอ้างเป็นเหตุขอขยายเวลา
</br>ทำการตามสัญญา หรือของดหรือลดค่าปรับไม่ได้
    </p>
    <p class='tab4 t-16'>
        (๘) ในกรณีที่ผู้ขายส่งมอบคอมพิวเตอร์ถูกต้องแต่ไม่ครบจำนวน หรือส่งมอบครบจำนวนแต่ไม่ถูกต้อง</br>
ทั้งหมด ผู้ซื้อจะตรวจรับเฉพาะส่วนที่ถูกต้อง โดยออกหลักฐานการตรวจรับเฉพาะส่วนนั้นก็ได้ (ความในวรรคสามนี้ จะไม่</br>
กำหนดไว้ในกรณีที่ผู้ซื้อต้องการคอมพิวเตอร์ทั้งหมดในคราวเดียวกัน หรือการซื้อคอมพิวเตอร์ที่ประกอบเป็นชุดหรือหน่วย ถ้าขาดส่วนประกอบอย่างหนึ่งอย่างใดไปแล้ว จะไม่สามารถใช้งานได้โดยสมบูรณ์)
    </p>
    <p class='tab3 t-16'><b>ข้อ ๖ การชำระเงิน</b></p>
    <p class='tab4 t-16'>
        (๙) ผู้ซื้อจะชำระเงินค่าคอมพิวเตอร์ให้แก่ผู้ขาย เมื่อผู้ซื้อได้รับมอบคอมพิวเตอร์ตามข้อ ๕ ไว้โดยครบ</br>ถ้วนแล้ว
    </p>
    <p class='tab4 t-16'>
        (๑๐) ผู้ซื้อตกลงชำระเงินค่าคอมพิวเตอร์ให้แก่ผู้ขาย ดังนี้
    </p>
    <p class='tab4 t-16'>
        ๖.๑ เงินล่วงหน้า จำนวน {result.AdvancePayment?.ToString("N2") ?? "0.00"} บาท ({CommonDAO.NumberToThaiText(result.AdvancePayment ?? 0)}) จะจ่ายให้ภายใน {result.PaymentDueDays} วัน </br>นับถัดจากวันลงนามในสัญญา ทั้งนี้ โดยผู้ขายจะต้องนำหลักประกันการรับเงินล่วงหน้าเป็น {result.PaymentGuaranteeType} {result.PaymentGuaranteeTypeOther} เต็มตามจำนวนเงินล่วงที่ได้รับ 
</br>มามอบให้แก่ผู้ซื้อเป็นหลักประกันการชำระคืนเงินล่วงหน้าก่อนการรับชำระเงินล่วงหน้านั้น 
</br>และผู้ซื้อจะคืนหลักประกันการรับเงินล่วงหน้าให้แก่ผู้ขาย เมื่อผู้ซื้อจ่ายเงินที่เหลือตามข้อ ๖.๒ แล้ว 
</br>เว้นแต่ในกรณีดังต่อไปนี้ ผู้ขายมีสิทธิขอคืนหลักประกันการรับเงินล่วงหน้าบางส่วนก่อนได้
    </p>
    <p class='tab4 t-16'>
        (๑) กรณีผู้ขายได้วางหลักประกันการรับเงินล่วงหน้าไว้ฉบับเดียว หากผู้ซื้อได้หักเงินล่วงหน้าไปแล้ว ผู้ขายมีสิทธิขอคืนหลักประกันการรับเงินล่วงหน้าในส่วนที่ผู้ซื้อได้หักเงินล่วงหน้าไปแล้วนั้น โดยผู้ขายจะต้องนำหลักประกันการรับเงินล่วงหน้าฉบับใหม่ที่มีมูลค่าเท่ากับเงินล่วงหน้าที่เหลืออยู่มาวางให้แก่ผู้ซื้อ
    </p>
    <p class='tab4 t-16'>
        (๒) กรณีผู้ขายได้วางหลักประกันการรับเงินล่วงหน้าไว้หลายฉบับ ซึ่งแต่ละฉบับมีมูลค่าเท่ากับจำนวนเงิน</br>
ล่วงหน้าที่ผู้ซื้อจะต้องหักไว้ในแต่ละงวด หากผู้ซื้อได้หักเงินล่วงหน้าในงวดใดแล้วผู้ขายมีสิทธิขอคืนหลักประกันการรับเงิน</br>ล่วงหน้าในงวดนั้นได้
    </p>
    <p class='tab4 t-16'>
        ๖.๒ เงินที่เหลือ จำนวน {result.RemainingPaymentAmount?.ToString("N2") ?? "0.00"} บาท ({CommonDAO.NumberToThaiText(result.RemainingPaymentAmount ?? 0)}) จะจ่ายให้เมื่อผู้ซื้อได้รับคอมพิวเตอร์ตามข้อ ๕ ไว้โดยถูกต้องครบถ้วน และผู้ขายได้อบรมเจ้าหน้าที่ของผู้ซื้อตามข้อ ๑๐ เสร็จสิ้นแล้ว
    </p>
    <p class='tab4 t-16'>
        (๑๑) การจ่ายเงินตามเงื่อนไขแห่งสัญญานี้ ผู้ซื้อจะโอนเงินเข้าบัญชีเงินฝากธนาคารของผู้ขาย ชื่อธนาคาร {result.SaleBankName} สาขา {result.SaleBankBranch} ชื่อบัญชี {result.SaleBankAccountName} เลขที่บัญชี {result.SaleBankAccountNumber} ทั้งนี้ ผู้ขายตกลงเป็นผู้รับภาระเงินค่าธรรมเนียม หรือค่าบริการอื่นใดเกี่ยวกับการโอน รวมทั้งค่าใช้จ่ายอื่นใด(ถ้ามี) ที่ธนาคาร</br>เรียกเก็บและยินยอมให้มีการหักเงินดังกล่าวจากจำนวนเงินโอนในงวดนั้น ๆ
    </p>
    <p class='tab3 t-16'><b>ข้อ ๗ การรับประกันความชำรุดบกพร่อง</b></p>
    <p class='tab4 t-16'>
        ผู้ขายตกลงรับประกันความชำรุดบกพร่องหรือขัดข้องของคอมพิวเตอร์และการติดตั้งตามสัญญานี้เป็นเวลา {result.WarrantyPeriodYears} ปี {result.WarrantyPeriodMonths} เดือน นับถัดจากวันที่ผู้ซื้อได้รับมอบคอมพิวเตอร์ทั้งหมดโดยถูกต้องครบถ้วนตามสัญญา ถ้าภายในระยะเวลาดังกล่าว
</br>
คอมพิวเตอร์ชำรุดบกพร่องหรือขัดข้อง หรือใช้งานไม่ได้ทั้งหมดหรือแต่บางส่วน หรือเกิดความชำรุดบกพร่องหรือขัดข้อง</br>
จากการติดตั้ง เว้นแต่ความชำรุดบกพร่องหรือขัดข้องดังกล่าว เกิดขึ้นจากความผิดของผู้ซื้อซึ่งไม่ได้เกิดขึ้นจากการใช้งาน</br>
ตามปกติ ผู้ขายจะต้องจัดการซ่อมแซมแก้ไขให้อยู่ในสภาพใช้การได้ดีดังเดิม โดยต้องเริ่มจัดการซ่อมแซมแก้ไขภายใน {result.DaysToRepairAfterNoti} วัน 
</br>นับถัดจากวันที่ได้รับแจ้งจากผู้ซื้อโดยไม่คิดค่าใช้จ่ายใด ๆ จากผู้ซื้อทั้งสิ้น ถ้าผู้ขายไม่จัดการซ่อมแซมแก้ไขภายในกำหนด
</br>เวลาดังกล่าว ผู้ซื้อมีสิทธิที่จะทำการนั้นเองหรือจ้างผู้อื่นทำการนั้นแทนผู้ขาย โดยผู้ขายต้องออกค่าใช้จ่ายเองทั้งสิ้นแทนผู้ซื้อ
    </p>
    <p class='tab4 t-16'>
        ผู้ขายมีหน้าที่บำรุงรักษาและซ่อมแซมแก้ไขคอมพิวเตอร์ให้อยู่ในสภาพใช้งานได้ดีอยู่เสมอ</br>
        ตลอดระยะเวลาดังกล่าวในวรรคหนึ่งด้วยค่าใช้จ่ายของผู้ขาย โดยให้มีเวลาคอมพิวเตอร์</br>
        ขัดข้องรวมตามเกณฑ์การคำนวณเวลาขัดข้องไม่เกินเดือนละ {result.MaximumDownTimeHours} ชั่วโมง </br>
        หรือร้อยละ {result.MaximumDownTimePercent} ของเวลาใช้งานทั้งหมดของคอมพิวเตอร์ของเดือนนั้น</br> 
        แล้วแต่ตัวเลขใดจะมากกว่ากัน มิฉะนั้นผู้ขายต้องยอมให้ผู้ซื้อคิดค่าปรับเป็นรายชั่วโมง </br>
        ในอัตราร้อยละ {result.PenaltyPerHourPercent} ของราคาคอมพิวเตอร์ทั้งหมด</br>
        ตามสัญญานี้คิดเป็นเงิน {result.PenaltyPerHour} บาท ({CommonDAO.NumberToThaiText(result.PenaltyPerHour ?? 0)}) ต่อชั่วโมง ในช่วงเวลาที่ไม่สามารถใช้คอมพิวเตอร์ได้ในส่วนที่เกินกว่ากำหนดเวลาขัดข้องข้างต้น
    </p>
    <p class='tab4 t-16'> เกณฑ์การคำนวณเวลาขัดข้องของคอมพิวเตอร์ตามวรรคสอง ให้เป็นดังนี้</p>
    <p class='tab4 t-16'> - กรณีที่คอมพิวเตอร์เกิดขัดข้องพร้อมกันหลายหน่วย ให้นับเวลาขัดข้องของหน่วยที่มีตัวถ่วง</br>มากที่สุดเพียงหน่วยเดียว</p>
    <p class='tab4 t-16'> - กรณีความเสียหายอันสืบเนื่องมาจากความขัดข้องของคอมพิวเตอร์แตกต่างกัน เวลาที่ใช้ในการคำนวณ</br>ค่าปรับจะเท่ากับเวลาขัดข้องของคอมพิวเตอร์หน่วยนั้นคูณด้วยตัวถ่วงซึ่งมีค่าต่างๆ ตามเอกสารแนบท้ายสัญญาผนวก ๔</p>
    <p class='tab4 t-16'>ผู้ขายจะต้องชำระค่าปรับตามวรรคสองให้แก่ผู้ซื้อภายใน {result.PenaltyDueDaysIn} วัน</br>
    นับถัดจากวันที่ได้รับแจ้งเป็นหนังสือจากผู้ซื้อ หากผู้ขายไม่ชำระค่าปรับตามที่ได้รับแจ้งจากผู้ซื้อภายในเวลา</br>
    ที่กำหนดผู้ซื้อมีสิทธิบังคับเอาจากหลักประกันการปฏิบัติตามสัญญาได้</p>
    <p class='tab4 t-16'> การที่ผู้ซื้อทำการนั้นเอง หรือให้ผู้อื่นทำการนั้นแทนผู้ขาย ไม่ทำให้ผู้ขายหลุดพ้นจาก ความรับผิดตาม</br>
สัญญา หากผู้ขายไม่ชดใช้ค่าใช้จ่ายหรือค่าเสียหายตามที่ผู้ซื้อเรียกร้องผู้ซื้อมีสิทธิบังคับจากหลักประกันการปฏิบัติตาม</br>สัญญาได้</p>
    <p class='tab3 t-16'><b>ข้อ ๘ หลักประกันการปฏิบัติตามสัญญา</b></p>
    <p class='tab4 t-16'>
        ในขณะทำสัญญานี้ผู้ขายได้นำหลักประกันเป็น {result.PerformanceGuarantee} เป็นจำนวนเงิน {result.GuaranteeAmount?.ToString("N2") ?? "0.00"} บาท ({CommonDAO.NumberToThaiText(result.GuaranteeAmount ?? 0)}) ซึ่งเท่ากับร้อยละ {result.GuaranteePercent} ของราคาซื้อขายคอมพิวเตอร์ตาม
ข้อ ๑ มามอบให้แก่ผู้ซื้อเพื่อ</br>เป็นหลักประกันการปฏิบัติตามสัญญานี้
    </p>
    <p class='tab4 t-16'>
        (๑๖) กรณีผู้ขายใช้หนังสือค้ำประกันมาเป็นหลักประกันการปฏิบัติตามสัญญา หนังสือค้ำประกันดังกล่าว</br>
จะต้องออกโดยธนาคารที่ประกอบกิจการในประเทศไทย หรือโดยบริษัทเงินทุนหรือบริษัทเงินทุนหลักทรัพย์ที่ได้รับอนุญาต</br>
ให้ประกอบกิจการเงินทุนเพื่อการพาณิชย์และประกอบธุรกิจค้ำประกันตามประกาศของธนาคารแห่งประเทศไทย ตามราย</br>ชื่อ
บริษัทเงินทุนที่ธนาคารแห่งประเทศไทยแจ้งเวียนให้ทราบตามแบบที่คณะกรรมการนโยบายการจัดซื้อจัดจ้างและ</br>การบริหาร
พัสดุภาครัฐกำหนด หรืออาจเป็นหนังสือค้ำประกันอิเล็กทรอนิกส์ตามวิธีการที่กรมบัญชีกลางกำหนดก็ได้และ</br>จะต้องมีอายุการค้ำประกันตลอดไปจนกว่าผู้ขายพ้นข้อผูกพันตามสัญญานี้
    </p>
    <p class='tab4 t-16'>
        หลักประกันที่ผู้ขายนำมามอบให้ตามวรรคหนึ่ง จะต้องมีอายุครอบคลุมความรับผิดทั้งปวงของผู้ขายตลอด</br>
อายุสัญญา ถ้าหลักประกันที่ผู้ขายนำมามอบให้ดังกล่าวลดลงหรือเสื่อมค่าลง หรือมีอายุไม่ครอบคลุมถึงความรับผิดของ</br>
ผู้ขายตลอดอายุสัญญาไม่ว่าด้วยเหตุใดๆ ก็ตาม รวมถึงกรณีผู้ขายส่งมอบและติดตั้งคอมพิวเตอร์ล่าช้าเป็นเหตุให้ระยะเวลา</br>
แล้วเสร็จ หรือวันครบกำหนดความรับผิดในความชำรุดบกพร่องตามสัญญาเปลี่ยนแปลงไป ไม่ว่าจะเกิดขึ้นคราวใด </br>
ผู้ขายต้องหาหลักประกันใหม่หรือหลักประกันเพิ่มเติมให้มีจำนวนครบถ้วนตามวรรคหนึ่ง มามอบให้แก่ผู้ซื้อภายใน {result.NewGuaranteeDays} วัน</br>
นับถัดจากวันที่ได้รับแจ้งเป็นหนังสือจากผู้ซื้อ
    </p>
    <p class='tab4 t-16'>
        หลักประกันที่ผู้ขายนำมามอบไว้ตามข้อนี้ ผู้ซื้อจะคืนให้แก่ผู้ขายโดยไม่มีดอกเบี้ยเมื่อผู้ขายพ้นจาก</br>ข้อผูกพัน และความรับผิดทั้งปวงตามสัญญานี้แล้ว
    </p>
    <p class='tab3 t-16'><b>ข้อ ๙ การโอนกรรมสิทธิ์</b></p>
    <p class='tab4 t-16'>
        คู่สัญญาตกลงกันว่ากรรมสิทธิ์ในคอมพิวเตอร์ตามสัญญาจะโอนไปยังผู้ซื้อเมื่อผู้ซื้อได้รับมอบคอมพิวเตอร์</br>ดังกล่าวตามข้อ ๕ แล้ว
    </p>
    <p class='tab3 t-16'><b>ข้อ ๑๐ การอบรม</b></p>
    <p class='tab4 t-16'>
        ผู้ขายต้องจัดอบรมวิชาการด้านคอมพิวเตอร์ให้แก่เจ้าหน้าที่ของผู้ซื้อจนสามารถใช้งานคอมพิวเตอร์ได้อย่าง</br>
มีประสิทธิภาพ โดยต้องดำเนินการฝึกอบรมให้แล้วเสร็จภายใน {result.TrainingPeriodDays} วันนับถัดจากวันที่ผู้ซื้อได้รับมอบคอมพิวเตอร์ โดยไม่คิดค่าใช้จ่ายใด ๆ รายละเอียดของการฝึกอบรมให้เป็นไปตามเอกสารแนบท้ายสัญญาผนวก ๕
    </p>
    <p class='tab3 t-16'><b>ข้อ ๑๑ คู่มือการใช้คอมพิวเตอร์</b></p>
    <p class='tab4 t-16'>
        ผู้ขายต้องจัดหาและส่งมอบคู่มือการใช้คอมพิวเตอร์ตามสัญญานี้ ตามที่กำหนดไว้ในเอกสารแนบท้าย</br>สัญญาผนวก ๖ จำนวน {result.ComputerManualsCount} ชุด ให้กับผู้ซื้อในวันที่ส่งมอบคอมพิวเตอร์ พร้อมทั้งปรับปรุงให้ทันสมัยเป็นปัจจุบันตลอด</br>อายุสัญญานี้ ทั้งนี้ โดยไม่คิดเงินเพิ่มจากผู้ซื้อ
    </p>
    <p class='tab3 t-16'><b>ข้อ ๑๒ การรับประกันความเสียหาย</b></p>
    <p class='tab4 t-16'>
        ในกรณีที่บุคคลภายนอกกล่าวอ้างหรือใช้สิทธิเรียกร้องใดๆ ว่ามีการละเมิดลิขสิทธิ์หรือสิทธิบัตรหรือ</br>
สิทธิใดๆ เกี่ยวกับคอมพิวเตอร์ตามสัญญานี้ โดยผู้ซื้อมิได้แก้ไขดัดแปลงไปจากเดิม ผู้ขายจะต้องดำเนินการทั้งปวงเพื่อ</br>
ให้การกล่าวอ้างหรือการเรียกร้องดังกล่าวระงับสิ้นไปโดยเร็ว หากผู้ซื้อต้องรับผิดชดใช้ค่าเสียหายต่อบุคคลภายนอก
เนื่องจากผลแห่งการละเมิดลิขสิทธิ์หรือสิทธิบัตรหรือสิทธิใด ๆ ดังกล่าว ผู้ขายต้องเป็นผู้ชำระค่าเสียหายและค่าใช้จ่ายรวม</br>
ทั้งค่าฤชาธรรมเนียมและค่าทนายความแทนผู้ซื้อ ทั้งนี้ ผู้ซื้อจะแจ้งให้ผู้ขายทราบเป็นหนังสือในเมื่อได้มีการกล่าวอ้างหรือ</br>ใช้สิทธิเรียกร้องดังกล่าวโดยไม่ชักช้า
    </p>
    <p class='tab3 t-16'><b>ข้อ ๑๓ การบอกเลิกสัญญา</b></p>
    <p class='tab4 t-16'>
        เมื่อครบกำหนดส่งมอบคอมพิวเตอร์ตามสัญญาแล้ว หากผู้ขายไม่ส่งมอบและติดตั้งคอมพิวเตอร์บาง</br>
รายการหรือทั้งหมดให้แก่ผู้ซื้อภายในกำหนดเวลาดังกล่าว หรือส่งมอบคอมพิวเตอร์ไม่ตรงตามสัญญาหรือมีคุณสมบัติ</br>
ไม่ถูกต้องตามสัญญา หรือส่งมอบและติดตั้งแล้วเสร็จภายในกำหนดแต่ไม่สามารถใช้งานได้อย่างมีประสิทธิภาพ หรือใช้งาน</br>
ไม่ได้ครบถ้วนตามสัญญา หรือผู้ขายไม่ปฏิบัติตามสัญญาข้อใดข้อหนึ่ง ผู้ซื้อมีสิทธิบอกเลิกสัญญาทั้งหมดหรือแต่บางส่วนได้ การใช้สิทธิบอกเลิกสัญญานั้นไม่กระทบถึงสิทธิของผู้ซื้อที่จะเรียกร้องค่าเสียหายจากผู้ขาย
    </p>
    <p class='tab4 t-16'>
        ในกรณีที่ผู้ซื้อใช้สิทธิบอกเลิกสัญญา ผู้ซื้อมีสิทธิริบหรือบังคับจากหลักประกันตาม (๑๗) (ข้อ ๖ และ) 
ข้อ ๘ เป็นจำนวนเงินทั้งหมดหรือแต่บางส่วนก็ได้ แล้วแต่ผู้ซื้อจะเห็นสมควร และถ้าผู้ซื้อจัดซื้อคอมพิวเตอร์รวมถึง
การติดตั้ง</br>
จากบุคคลอื่นเต็มจำนวนหรือเฉพาะจำนวนที่ขาดส่ง แล้วแต่กรณี ภายในกำหนด {result.TeminationNewMonths} วัน นับถัดจากวันบอกเลิกสัญญา 
</br>ผู้ขายจะต้องชดใช้ราคาที่เพิ่มขึ้นจากราคาที่กำหนดไว้ในสัญญานี้ด้วย รวมทั้งค่าใช้จ่ายใดๆ ที่ผู้ซื้อต้องใช้จ่าย
</br>ในการจัดหาผู้ขายรายใหม่ดังกล่าวด้วย
    </p>
    <p class='tab4 t-16'>
        ในกรณีที่ผู้ขายได้ส่งมอบคอมพิวเตอร์ให้แก่ผู้ซื้อและผู้ซื้อบอกเลิกสัญญา ผู้ขายจะต้องนำคอมพิวเตอร์</br>
กลับคืนไป และทำสถานที่ที่รื้อถอนคอมพิวเตอร์ออกไปให้มีสภาพดังที่มีอยู่เดิมก่อนทำสัญญานี้ภายใน {result.ReturnDaysIn} วัน นับถัดจาก
</br>วันที่ผู้ซื้อบอกเลิกสัญญา โดยผู้ขายเป็นผู้เสียค่าใช้จ่ายเองทั้งสิ้น
    </p>
    <p class='tab4 t-16'>
        (19)ถ้าผู้ขายไม่ยอมนำคอมพิวเตอร์กลับคืนไปภายในกำหนดเวลาดังกล่าวตามวรรคสาม ผู้ซื้อจะกำหนด</br>
เวลาให้ผู้ขายนำคอมพิวเตอร์กลับคืนไปอีกครั้งหนึ่ง หากพ้นกำหนดเวลาดังกล่าวแล้ว ผู้ขายยังไม่นำคอมพิวเตอร์กลับคืน</br>
ไปอีก ผู้ซื้อมีสิทธินำคอมพิวเตอร์ออกขายทอดตลาด เงินที่ได้จากการขายทอดตลาด ผู้ขายยอมให้ผู้ซื้อหักเป็นค่าปรับ</br>
และหักเป็นค่าใช้จ่าย และค่าเสียหายที่เกิดแก่ผู้ซื้อ ซึ่งรวมถึงค่าใช้จ่ายต่างๆ ที่ผู้ซื้อได้เสียไปในการดำเนินการขายทอด</br>
ตลาดคอมพิวเตอร์ดังกล่าว ค่าใช้จ่ายในการทำสถานที่ที่รื้อถอนคอมพิวเตอร์ออกไปให้มีสภาพดังที่มีอยู่เดิมก่อนทำสัญญานี้ เงินที่เหลือจากการหักค่าปรับ ค่าใช้จ่าย และค่าเสียหายดังกล่าวแล้วผู้ซื้อจะคืนให้แก่ผู้ขาย
    </p>
    <p class='tab4 t-16'>
        เมื่อผู้ซื้อบอกเลิกสัญญาแล้ว ผู้ซื้อไม่ต้องรับผิดชอบในความเสียหายใดๆ ทั้งสิ้นอันเกิดแก่คอมพิวเตอร์</br>ซึ่งอยู่ในความครอบครองของผู้ซื้อ
    </p>
    <p class='tab3 t-16'><b>ข้อ ๑๔ ค่าปรับ</b></p>
    <p class='tab4 t-16'>
        ในกรณีที่ผู้ซื้อยังไม่ใช้สิทธิบอกเลิกสัญญาตามข้อ ๑๓ ผู้ขายจะต้องชำระค่าปรับให้ผู้ซื้อเป็นรายวัน ในอัตราร้อยละ {result.FinePerDaysPercent} ของราคาคอมพิวเตอร์ที่ยังไม่ได้รับมอบ นับถัดจากวันครบกำหนดตามสัญญาจนถึงวันที่ผู้ขายได้</br>นำคอมพิวเตอร์มาส่งมอบและติดตั้งให้แก่ผู้ซื้อจนถูกต้องครบถ้วนตามสัญญา
    </p>
    <p class='tab4 t-16'>
        การคิดค่าปรับในกรณีที่คอมพิวเตอร์ที่ตกลงซื้อขายเป็นระบบ ถ้าผู้ขายส่งมอบเพียงบางส่วนหรือขาดส่วน</br>
ประกอบส่วนหนึ่งส่วนใดไป หรือส่งมอบและติดตั้งทั้งหมดแล้วแต่ใช้งานไม่ได้ถูกต้องครบถ้วน ให้ถือว่ายังไม่ได้ส่งมอบ</br>คอมพิวเตอร์นั้นเลย และคิดค่าปรับจากราคาคอมพิวเตอร์ทั้งระบบ
    </p>
    <p class='tab4 t-16'>
        ในระหว่างที่ผู้ซื้อยังไม่ได้ใช้สิทธิบอกเลิกสัญญานั้น หากผู้ซื้อเห็นว่าผู้ขายไม่อาจปฏิบัติตามสัญญาต่อไปได้ </br>
ผู้ซื้อจะใช้สิทธิบอกเลิกสัญญา และริบหรือบังคับจากหลักประกันตาม (21) (ข้อ ๖ และ) ข้อ ๘ กับเรียกร้องให้ชดใช้ราคา</br>
ที่เพิ่มขึ้นตามที่กำหนดไว้ในข้อ ๑๓ วรรคสอง ก็ได้ และถ้าผู้ซื้อได้แจ้งข้อเรียกร้องให้ชำระค่าปรับไปยังผู้ขายเมื่อครบกำหนด</br>ส่งมอบแล้ว ผู้ซื้อมีสิทธิที่จะปรับผู้ขายจนถึงวันบอกเลิกสัญญาได้อีกด้วย
    </p>
    <p class='tab3 t-16'><b>ข้อ ๑๕ การบังคับค่าปรับ ค่าเสียหาย และค่าใช้จ่าย</b></p>
    <p class='tab4 t-16'>
        ในกรณีที่ผู้ขายไม่ปฏิบัติตามสัญญาข้อใดข้อหนึ่งด้วยเหตุใด ๆ ก็ตาม จนเป็นเหตุให้เกิดค่าปรับ ค่าเสียหาย 
หรือค่าใช้จ่ายแก่ผู้ซื้อ ผู้ขายต้องชดใช้ค่าปรับ ค่าเสียหาย หรือค่าใช้จ่ายดังกล่าวให้แก่ผู้ซื้อ โดยสิ้นเชิงภายในกำหนด {result.EnforcementOfFineDays} วัน 
นับถัดจากวันที่ได้รับแจ้งเป็นหนังสือจากผู้ซื้อ หากผู้ขายไม่ชดใช้ให้ถูกต้องครบถ้วนภายในระยะเวลาดังกล่าวให้ผู้ซื้อมีสิทธิ</br>
ที่จะหักเอาจากค่าคอมพิวเตอร์ที่ต้องชำระ หรือบังคับจากหลักประกันการปฏิบัติตามสัญญาได้ทันที
    </p>
    <p class='tab4 t-16'>
        หากค่าปรับ ค่าเสียหาย หรือค่าใช้จ่ายที่บังคับจากค่าคอมพิวเตอร์ที่ต้องชำระหรือหลักประกันการปฏิบัติ</br>
ตามสัญญาแล้วยังไม่เพียงพอ ผู้ขายยินยอมชำระส่วนที่เหลือที่ยังขาดอยู่จนครบถ้วนตามจำนวนค่าปรับ ค่าเสียหาย หรือค่า</br>
ใช้จ่ายนั้น ภายในกำหนด {result.OutstandingPeriodDays} วัน นับถัดจากวันที่ได้รับแจ้งเป็นหนังสือจากผู้ซื้อ
    </p>
    <p class='tab4 t-16'>
        หากมีเงินค่าคอมพิวเตอร์ที่ซื้อขายตามสัญญาที่หักไว้จ่ายเป็นค่าปรับ ค่าเสียหาย หรือค่าใช้จ่ายแล้วยัง</br>เหลืออยู่อีกเท่าใด ผู้ซื้อจะคืนให้แก่ผู้ขายทั้งหมด
    </p>
    <p class='tab3 t-16'><b>ข้อ ๑๖ การงดหรือลดค่าปรับ หรือขยายเวลาในการปฏิบัติตามสัญญา</b></p>
    <p class='tab4 t-16'>
        ในกรณีที่มีเหตุเกิดจากความผิดหรือความบกพร่องของผู้ซื้อ หรือเหตุสุดวิสัย หรือเกิดจากพฤติการณ์</br>
อันหนึ่งอันใดที่ผู้ขายไม่ต้องรับผิดตามกฎหมายหรือเหตุอื่นตามที่กำหนดในกฎกระทรวง ซึ่งออกตามความในกฎหมายว่า</br>
ด้วยการจัดซื้อจัดจ้างและการบริหารพัสดุภาครัฐ ทำให้ผู้ขายไม่สามารถส่งมอบและติดตั้งคอมพิวเตอร์ หรือบำรุงรักษา
และซ่อมแซมแก้ไขคอมพิวเตอร์ ตามเงื่อนไขและกำหนดเวลาแห่งสัญญานี้ได้ ผู้ขายมีสิทธิของดหรือลดค่าปรับหรือขยาย</br>
เวลาทำการตามสัญญา โดยจะต้องแจ้งเหตุหรือพฤติการณ์ดังกล่าวพร้อมหลักฐานเป็นหนังสือให้ผู้ซื้อทราบภายใน ๑๕ (สิบห้า) วันนับถัดจากวันที่เหตุนั้นสิ้นสุดลง หรือตามที่กำหนดในกฎกระทรวงดังกล่าว
    </p>
    <p class='tab4 t-16'>
        ถ้าผู้ขายไม่ปฏิบัติให้เป็นไปตามความในวรรคหนึ่ง ให้ถือว่าผู้ขายได้สละสิทธิเรียกร้องในการที่จะของด</br>
หรือลดค่าปรับหรือขยายเวลาทำการตามสัญญา โดยไม่มีเงื่อนไขใด ๆ ทั้งสิ้น เว้นแต่กรณีเหตุเกิดจากความผิดหรือ</br>
ความบกพร่องของฝ่ายผู้ซื้อซึ่งมีหลักฐานชัดแจ้งหรือผู้ซื้อทราบดีอยู่แล้วตั้งแต่ต้น
    </p>
    <p class='tab4 t-16'>
    ...
    <p class='tab3 t-16'><b>ข้อ ๑๗ การใช้เรือไทย</b></p>
    <p class='tab4 t-16'>
        ถ้าผู้ขายจะต้องสั่งหรือนำเข้าคอมพิวเตอร์มาจากต่างประเทศและต้องนำเข้ามาโดยทางเรือในเส้นทางเดิน</br>
เรือที่มีเรือไทยเดินอยู่ และสามารถให้บริการรับขนได้ตามที่รัฐมนตรีว่าการกระทรวงคมนาคมประกาศกำหนด ผู้ขายต้อง</br>
จัดการให้คอมพิวเตอร์บรรทุกโดยเรือไทยหรือเรือที่มีสิทธิเช่นเดียวกับเรือไทยจากต่างประเทศมายังประเทศไทย เว้นแต่จะ</br>
ได้รับอนุญาตจากกรมเจ้าท่าก่อนบรรทุกคอมพิวเตอร์ลงเรืออื่นที่มิใช่เรือไทยหรือเป็นของที่รัฐมนตรีว่าการกระทรวงคมนาคม</br>
ประกาศยกเว้นให้บรรทุกโดยเรืออื่นได้ ทั้งนี้ ไม่ว่าการสั่งหรือนำเข้าคอมพิวเตอร์จากต่างประเทศจะเป็นแบบใด
    </p>
    <p class='tab4 t-16'>
        ในการส่งมอบคอมพิวเตอร์ให้แก่ผู้ซื้อ ถ้าเป็นกรณีตามวรรคหนึ่งผู้ขายจะต้องส่งมอบใบตราส่ง (Bill of
Lading) หรือสำเนาใบตราส่งสำหรับคอมพิวเตอร์นั้น ซึ่งแสดงว่าได้บรรทุกมาโดยเรือไทย หรือเรือที่มีสิทธิเช่นเดียวกับ</br>เรือไทยให้แก่ผู้ซื้อพร้อมกับการส่งมอบคอมพิวเตอร์ด้วย
    </p>
    <p class='tab4 t-16'>
        ในกรณีที่คอมพิวเตอร์ไม่ได้บรรทุกจากต่างประเทศมายังประเทศไทยโดยเรือไทยหรือเรือที่มีสิทธิเช่นเดียว</br>
กับเรือไทย ผู้ขายต้องส่งมอบหลักฐานซึ่งแสดงว่าได้รับอนุญาตจากกรมเจ้าท่าให้บรรทุกของโดยเรืออื่นได้ หรือหลักฐาน</br>
ซึ่งแสดงว่าได้ชำระค่าธรรมเนียมพิเศษเนื่องจากการไม่บรรทุกของโดยเรือ ไทยตามกฎหมายว่าด้วยการส่งเสริมการพาณิชย</br>นาวีแล้วอย่างใดอย่างหนึ่งแก่ผู้ซื้อด้วย
    </p>
    <p class='tab4 t-16'>
        ในกรณีที่ผู้ขายไม่ส่งมอบหลักฐานอย่างใดอย่างหนึ่งดังกล่าวในวรรคสองและวรรคสามให้แก่ผู้ซื้อแต่จะขอส่งมอบคอมพิวเตอร์ให้ผู้ซื้อก่อนโดยยังไม่รับชำระเงินค่าคอมพิวเตอร์ ผู้ซื้อมีสิทธิรับคอมพิวเตอร์ไว้ก่อนและชำระเงินค่าคอมพิวเตอร์เมื่อผู้ขายได้ปฏิบัติถูกต้องครบถ้วนดังกล่าวแล้วได้
    </p>
    <p class='tab4 t-16'>
    ...
    <p class='text-center t-22'><b>วิธีปฏิบัติเกี่ยวกับสัญญาซื้อขายคอมพิวเตอร์</b></p>
            ...
            <div class='tab2 t-16'> (๑) ให้ระบุเลขที่สัญญาในปีงบประมาณหนึ่งๆ ตามลำดับ</div>
            <div class='tab2 t-16'> (๒) ให้ระบุชื่อของหน่วยงานของรัฐที่เป็นนิติบุคคล เช่น กรม ก. หรือรัฐวิสาหกิจ ข. เป็นต้น</div>
            <div class='tab2 t-16'> (๓) ให้ระบุชื่อและตำแหน่งของหัวหน้าหน่วยงานของรัฐที่เป็นนิติบุคคลนั้น หรือผู้ที่ได้รับมอบอำนาจ เช่น นาย ก. อธิบดีกรม………...… หรือ นาย ข. ผู้ได้รับมอบอำนาจจากอธิบดีกรม………......………..</div>
            <div class='tab2 t-16'> (๔) ให้ระบุชื่อผู้รับจ้าง</div>
            <div class='tab5 t-16'> ก. กรณีนิติบุคคล เช่น ห้างหุ้นส่วนสามัญจดทะเบียน ห้างหุ้นส่วนจำกัด บริษัทจำกัด</div>
            <div class='tab5 t-16'> ข. กรณีบุคคลธรรมดา ให้ระบุชื่อและที่อยู่</div>
            <div class='tab2 t-16'> (๕) เป็นข้อความหรือเงื่อนไขเพิ่มเติม ซึ่งหน่วยงานของรัฐผู้ทำสัญญาอาจเลือกใช้หรือตัดออกได้ตามข้อเท็จจริง</div>
           <div class='tab2 t-16'>  (๖) เป็นข้อความหรือเงื่อนไขเพิ่มเติม ซึ่งหน่วยงานของรัฐผู้ทำสัญญาอาจเลือกใช้หรือตัดออกได้ตามข้อเท็จจริง</div>
          <div class='tab2 t-16'>   (๗) ให้ระบุยี่ห้อคอมพิวเตอร์ รุ่นคอมพิวเตอร์</div>
           <div class='tab2 t-16'>  (๘) เป็นข้อความหรือเงื่อนไขเพิ่มเติม ซึ่งหน่วยงานของรัฐผู้ทำสัญญาอาจเลือกใช้หรือตัดออกได้ตามข้อเท็จจริง</div>
            <div class='tab2 t-16'> (๙) เป็นข้อความหรือเงื่อนไขเพิ่มเติม ซึ่งหน่วยงานของรัฐผู้ทำสัญญาอาจเลือกใช้หรือตัดออกได้ตามข้อเท็จจริง</div>
           <div class='tab2 t-16'>  (๑๐) เป็นข้อความหรือเงื่อนไขเพิ่มเติม ซึ่งหน่วยงานของรัฐผู้ทำสัญญาอาจเลือกใช้หรือตัดออกได้ตาม ข้อเท็จจริง</div>
           <div class='tab2 t-16'>  (๑๑) เป็นข้อความหรือเงื่อนไขเพิ่มเติม ซึ่งหน่วยงานของรัฐผู้ทำสัญญาอาจเลือกใช้หรือตัดออกได้ตาม ข้อเท็จจริง</div>
           <div class='tab2 t-16'>  (๑๒) ระยะเวลารับประกันและระยะเวลาแก้ไขซ่อมแซมจะกำหนดเท่าใด แล้วแต่ลักษณะของสิ่งของ
ที่ซื้อขายกันโดยให้อยู่ในดุลพินิจของผู้ซื้อ ทั้งนี้ จะต้องประกาศให้ทราบในเอกสารประกวดราคาด้วย</div>
           <div class='tab2 t-16'>  (๑๓) ให้กำหนดในอัตราระหว่างร้อยละ ๐.๐๒๕ – ๐.๐๓๕ ของราคาตามสัญญาต่อชั่วโมง</div>
           <div class='tab2 t-16'>  (๑๔) “หลักประกัน” หมายถึง หลักประกันที่ผู้รับจ้างนำมามอบไว้แก่หน่วยงานของรัฐเมื่อลงนาม
ในสัญญาเพื่อประกันความเสียหายที่อาจเกิดขึ้นจากการปฏิบัติตามสัญญา ดังนี้</div>
            <div class='tab2 t-16'> (๑)เงินสด</div>
            <div class='tab2 t-16'> (๒)เช็คหรือดราฟท์ที่ธนาคารเซ็นสั่งจ่าย ซึ่งเป็นเช็คหรือดราฟท์ลงวันที่ที่ใช้เช็คหรือดราฟท์นั้นชำระต่อเจ้าหน้าที่
หรือก่อนวันนั้นไม่เกิน ๓ วันทำการ</div>
            <div class='tab2 t-16'> (๓)หนังสือค้ำประกันของธนาคารภายในประเทศตามตัวอย่างที่คณะกรรมการนโยบายกำหนด โดยอาจเป็นหนังสือค้ำประกันอิเล็กทรอนิกส์ตามวิธีการที่กรมบัญชีกลางกำหนดก็ได้</div>
            <div class='tab2 t-16'> (๔)หนังสือค้ำประกันของบริษัทเงินทุนหรือบริษัทเงินทุนหลักทรัพย์ที่ได้รับอนุญาตให้ประกอบกิจการเงินทุนเพื่อ
การพาณิชย์และประกอบธุรกิจค้ำประกันตามประกาศของธนาคารแห่งประเทศไทย ตามรายชื่อบริษัทเงินทุนที่ธนาคารแห่งประเทศไทยแจ้งเวียนให้ทราบ
โดยอนุโลมให้ใช้ตามตัวอย่างหนังสือค้ำประกันของธนาคารที่คณะกรรมการนโยบายกำหนด</div>
            <div class='tab2 t-16'> (๕)พันธบัตรรัฐบาลไทย</div>
            <div class='tab2 t-16'> (๑๕) ให้กำหนดจำนวนเงินหลักประกันการปฏิบัติตามสัญญาตามระเบียบกระทรวงการคลังว่าด้วย การจัดซื้อจัดจ้าง
และการบริหารพัสดุภาครัฐ พ.ศ. ๒๕๖๐ ข้อ ๑๖๘ ทั้งนี้ จะต้องประกาศให้ทราบ ในเอกสารประกวดราคาด้วย</div>
            <div class='tab2 t-16'> (๑๖) เป็นข้อความหรือเงื่อนไขเพิ่มเติม ซึ่งหน่วยงานของรัฐผู้ทำสัญญาอาจเลือกใช้หรือตัดออกได้ตาม ข้อเท็จจริง</div>
            <div class='tab2 t-16'>  (๑๗) เป็นข้อความหรือเงื่อนไขเพิ่มเติม ซึ่งหน่วยงานของรัฐผู้ทำสัญญาอาจเลือกใช้หรือตัดออกได้ตาม ข้อเท็จจริง</div>
            <div class='tab2 t-16'> (๑๘) กำหนดเวลาที่ผู้ซื้อจะซื้อสิ่งของจากแหล่งอื่นเมื่อบอกเลิกสัญญาและมีสิทธิเรียกเงินในส่วนที่เพิ่มขึ้น จากราคา
ที่กำหนดไว้ในสัญญานั้น ให้อยู่ในดุลพินิจของผู้ซื้อโดยตกลงกับผู้ขาย</div>
            <div class='tab2 t-16'> (๑๙) ความในวรรคนี้อาจแก้ไขเปลี่ยนแปลงได้ตามความเหมาะสมถ้าหน่วยงานของรัฐผู้ทำสัญญา สามารถกำหนด
มาตรการอื่นใดในสัญญา หรือกำหนดทางปฏิบัติเพื่อแก้ปัญหาที่ผู้ขายไม่ยอมนำคอมพิวเตอร์กลับคืนไปได้</div>
            <div class='tab2 t-16'> (๒๐) อัตราค่าปรับตามสัญญาข้อ ๑๔ ให้กำหนดเป็นรายวันในอัตราตายตัวระหว่างร้อยละ ๐.๐๑ – ๐.๒๐
ของราคาพัสดุที่ยังไม่ได้ส่งมอบ ตามระเบียบกระทรวงการคลังว่าด้วยการจัดซื้อจัดจ้างและการบริหารพัสดุภาครัฐ พ.ศ. ๒๕๖๐ ข้อ ๑๖๒
ส่วนกรณีจะปรับร้อยละเท่าใด ให้อยู่ในดุลพินิจของหน่วยงานของรัฐผู้ซื้อที่จะพิจารณา โดยคำนึงถึงราคาและลักษณะของพัสดุที่ซื้อ ซึ่งอาจมี
ผลกระทบต่อการที่ผู้ขายจะหลีกเลี่ยงไม่ปฏิบัติตามสัญญา แต่ทั้งนี้ การที่จะกำหนดค่าปรับเป็นร้อยละเท่าใดจะต้องกำหนดไว้ในเอกสารเชิญชวนด้วย</div>
            <div class='tab2 t-16'> (๒๑) เป็นข้อความหรือเงื่อนไขเพิ่มเติม ซึ่งหน่วยงานของรัฐผู้ทำสัญญาอาจเลือกใช้หรือตัดออกได้ตาม ข้อเท็จจริง
    </div>
</div>
        ...
        <head>
            <meta charset='utf-8'>
             ...
             <style>
                ...
                .tab2 {{ text-indent: 144px;  }}
                .tab3 {{ text-indent: 96px;  }}
                .tab4 {{ text-indent: 144px;  }}
                .tab5 {{ text-indent: 192px; }}
                .d-flex {{ display: flex; }}
                .w-100 {{ width: 100%; }}
                .w-40 {{ width: 40%; }}
                .w-50 {{ width: 50%; }}
