...
EContractDAO eContractDAO
    ...
    public async Task<byte[]> OnGetWordContact_ConsultantService_ToPDF(string id,string typeContact)
    {
        try
        {
            var result = await _e.GetCTRAsync(id);
            ...
            else
            {
                ...
                string ToThaiNumberText(decimal? number) => CommonDAO.NumberToThaiText(number ?? 0);
...
<div class='contract'>
    ...
    <div class=' tab3 t-16'>
        ...
        ระหว่าง {result.Contract_Organization}
        โดย {result.SignatoryName}
        ซึ่งต่อไปในสัญญานี้เรียกว่า “ผู้ว่าจ้าง” ฝ่ายหนึ่ง กับ…{result.ContractorName}
    </div>
    ...
    <div class=' tab3 t-16'>
        1.1 ผู้ว่าจ้างตกลงจ้างและที่ปรึกษาตกลงรับจ้างปฏิบัติงานตามโครงการ {result.ProjectName}: {result.ProjectDesc} {result.ProjectReference} ตามข้อกำหนดและเงื่อนไขแห่งสัญญานี้รวมทั้งเอกสารแนบท้ายสัญญาผนวก .......ทั้งนี้ ที่ปรึกษาจะต้องปฏิบัติงานให้เป็นไปตามหลักวิชาการและมาตรฐานวิชาชีพทางด้าน {result.ConsultExpertise} และบทบัญญัติแห่งกฎหมาย ที่เกี่ยวข้อง
    </div>
    <div class=' tab3 t-16'>
        1.2 ที่ปรึกษาจะต้องเริ่มลงมือทำงานภายใน {ToThaiDate(result.ProjectStartDate)} และจะต้องดำเนินการตามสัญญานี้ให้แล้วเสร็จภายใน {ToThaiDate(result.ProjectEndDate)}
    </div>
...
{htmlDocAtt}
    ...
    <div class=' tab3 t-16'>
        ถ้าสิ่งใดหรือการอันหนึ่งอันใดที่มิได้ระบุไว้ในรายการละเอียดแนบท้ายสัญญานี้ แต่เป็นการอันจำเป็นต้องทำเพื่อให้งานแล้วเสร็จบริบูรณ์ถูกต้องหรือบรรลุผลตามวัตถุประสงค์แห่งสัญญานี้ ที่ปรึกษาต้องจัดทำการนั้นๆ ให้โดยไม่คิดเอาค่าเสียหาย ค่าใช้จ่ายหรือค่าตอบแทนเพิ่มเติมใดๆ ทั้งสิ้น
    </div>
    <div class=' tab3 t-16'><b>ข้อ 3 ค่าจ้างและการจ่ายเงิน</b></div>
    <div class=' tab3 t-16'>
        ผู้ว่าจ้างและที่ปรึกษาได้ตกลงราคาค่าจ้างตามสัญญานี้ เป็นจำนวนเงินทั้งสิ้น {result.ContractTotalAmount} บาท ({ToThaiNumberText(result.ContractTotalAmount)}) ซึ่งได้รวมภาษีมูลค่าเพิ่ม เป็นเงินจำนวน…………...…….……..บาท (………..………………….…) ตลอดจนภาษีอากรอื่นๆ และค่าใช้จ่ายทั้งปวงด้วยแล้ว
    </div>
    <div class=' tab3 t-16'>
        ค่าจ้างจะแบ่งออกเป็น {result.ContractInstallment} งวด ซึ่งแต่ละงวดจะจ่ายให้เมื่อที่ปรึกษาได้ปฏิบัติงานตามที่กำหนดในเอกสารแนบท้ายสัญญาผนวก .......และคณะกรรมการตรวจรับพัสดุได้พิจารณาแล้วเห็นว่าครบถ้วนถูกต้องและตรวจรับเรียบร้อยแล้ว
    </div>
    <div class=' tab3 t-16'>
        ผู้ว่าจ้างอาจจะยึดหน่วงเงินค่าจ้างงวดใดๆ ไว้ก็ได้ หากที่ปรึกษาปฏิบัติงานไม่เป็นไปตามสัญญา และจะจ่ายให้ต่อเมื่อที่ปรึกษาได้ทำการแก้ไขข้อบกพร่องนั้นแล้ว
    </div>
    <div class=' tab3 t-16'>
        (7)การจ่ายเงินตามเงื่อนไขแห่งสัญญานี้ ผู้ว่าจ้างจะโอนเงินเข้าบัญชีเงินฝากธนาคารของ ที่ปรึกษา ชื่อธนาคาร {result.ContractBankName} สาขา {result.ContractBankBranch} ชื่อบัญชี {result.ContractBankAccountName} เลขที่บัญชี {result.ContractBankAccountNumber} ทั้งนี้ ที่ปรึกษาตกลงเป็นผู้รับภาระเงินค่าธรรมเนียมหรือค่าบริการอื่นใดเกี่ยวกับการโอน รวมทั้งค่าใช้จ่ายอื่นใด (ถ้ามี)ที่ธนาคารเรียกเก็บและยินยอมให้มีการหักเงินดังกล่าวจากจำนวนเงินโอนในงวดนั้นๆ (ความในวรรคนี้ใช้สำหรับกรณีที่หน่วยงานของรัฐจะจ่ายเงินตรงให้แก่ที่ปรึกษา (ระบบ Direct Payment)โดยการโอนเงินเข้าบัญชีเงินฝากธนาคารของที่ปรึกษา ตามแนวทางที่กระทรวงการคลังหรือหน่วยงานของรัฐเจ้าของงบประมาณเป็นผู้กำหนด แล้วแต่กรณี)
    </div>
    <div class=' tab3 t-16'>
        ค่าใช้จ่ายส่วนที่เบิกคืนได้(ถ้ามี)ผู้ว่าจ้างจะจ่ายคืนให้แก่ที่ปรึกษาสำหรับค่าใช้จ่ายซึ่งที่ปรึกษาได้ใช้จ่ายไปตามความเป็นจริงตามเงื่อนไขที่กำหนดไว้ในเอกสารแนบท้ายสัญญาผนวก .....
    </div>
    <div class=' tab3 t-16'><b>(8)ข้อ 4 เงินค่าจ้างล่วงหน้า</b></div>
    <div class=' tab3 t-16'>
        ผู้ว่าจ้างตกลงจ่ายเงินค่าจ้างล่วงหน้าให้แก่ที่ปรึกษา เป็นจำนวนเงิน {result.PrepaidAmount} บาท ({ToThaiNumberText(result.PrepaidAmount)}) ซึ่งเท่ากับร้อยละ {result.PrepaidPercents} ของค่าจ้างตามสัญญา
    </div>
    <div class=' tab3 t-16'>
        เงินค่าจ้างล่วงหน้าดังกล่าวจะจ่ายให้ภายหลังจากที่ที่ปรึกษาได้วางหลักประกันการรับเงินค่าจ้างล่วงหน้าเป็น {result.GuaranteeType} (หนังสือค้ำประกันหรือหนังสือค้ำประกันอิเล็กทรอนิกส์ของธนาคารภายในประเทศ …………………………....เต็มตามจำนวนเงินค่าจ้างล่วงหน้านั้นให้แก่ผู้ว่าจ้าง ที่ปรึกษาจะต้องออกใบเสร็จรับเงินค่าจ้างล่วงหน้าตามแบบที่ผู้ว่าจ้างกำหนดให้และที่ปรึกษาตกลงที่จะกระทำตามเงื่อนไขอันเกี่ยวกับการใช้จ่ายและการใช้คืนเงินค่าจ้างล่วงหน้านั้น ดังต่อไปนี้
    </div>
    <div class=' tab3 t-16'>
        4.1.ที่ปรึกษาจะใช้เงินค่าจ้างล่วงหน้านั้นเพื่อเป็นค่าใช้จ่ายในการปฏิบัติงานตามสัญญาเท่านั้นหากที่ปรึกษาใช้จ่ายเงินค่าจ้างล่วงหน้าหรือส่วนใดส่วนหนึ่งของเงินค่าจ้างล่วงหน้านั้นในทางอื่นผู้ว่าจ้างอาจจะเรียกเงินค่าจ้างล่วงหน้านั้นคืนจากที่ปรึกษาหรือบังคับเอาจากหลักประกันการรับเงินค่าจ้างล่วงหน้าได้ทันที
    </div>
    <div class=' tab3 t-16'>
        4.2.เมื่อผู้ว่าจ้างเรียกร้อง ที่ปรึกษาต้องแสดงหลักฐานการใช้จ่ายเงินค่าจ้างล่วงหน้า เพื่อพิสูจน์ว่าได้เป็นไปตามข้อ ๔.๑ ภายในกำหนด 15 (สิบห้า)วัน นับถัดจากวันได้รับแจ้งจากผู้ว่าจ้าง หากที่ปรึกษาไม่อาจแสดงหลักฐานดังกล่าว ภายในกำหนด 15 (สิบห้า)วัน ผู้ว่าจ้างอาจเรียกเงินค่าจ้างล่วงหน้าคืนจากที่ปรึกษาหรือบังคับเอาจากหลักประกันการรับเงินค่าจ้างล่วงหน้าได้ทันที
    </div>
    <div class=' tab3 t-16'>
        4.3.ในการจ่ายเงินค่าจ้างให้แก่ที่ปรึกษาตามข้อ 3 ผู้ว่าจ้างจะหักชดใช้คืนเงินค่าจ้างล่วงหน้าในแต่ละงวดไว้จำนวนร้อยละ {result.PrepaidDeductPercent} ของจำนวนเงินค่าจ้างในแต่ละงวดจนกว่าจำนวนเงินที่หักไว้จะครบตามจำนวนเงินที่หักค่าจ้างล่วงหน้าที่ที่ปรึกษาได้รับไปแล้ว ยกเว้นค่าจ้างงวดสุดท้ายจะหักไว้เป็นจำนวนเท่ากับจำนวนเงินค่าจ้างล่วงหน้าที่เหลือทั้งหมด
    </div>
    <div class=' tab3 t-16'>
        4.4.เงินจำนวนใดๆ ก็ตามที่ที่ปรึกษาจะต้องจ่ายให้แก่ผู้ว่าจ้างเพื่อชำระหนี้หรือเพื่อชดใช้ความรับผิดต่างๆ ตามสัญญา ผู้ว่าจ้างจะหักเอาจากเงินค่าจ้างงวดที่จะจ่ายให้แก่ที่ปรึกษาก่อนที่จะหักชดใช้คืนเงินค่าจ้างล่วงหน้า
    </div>
    <div class=' tab3 t-16'>
        4.5.ในกรณีที่มีการบอกเลิกสัญญา หากเงินค่าจ้างล่วงหน้าที่เหลือเกินกว่าจำนวนเงินที่ ที่ปรึกษาจะได้รับหลังจากหักชดใช้ในกรณีอื่นแล้ว ที่ปรึกษาจะต้องจ่ายคืนเงินจำนวนที่เหลือนั้นให้แก่ผู้ว่าจ้าง ภายใน 7 (เจ็ด)วัน นับถัดจากวันได้รับแจ้งเป็นหนังสือจากผู้ว่าจ้าง
    </div>
    <div class=' tab3 t-16'>
        4.6.ผู้ว่าจ้างจะคืนหลักประกันเงินค่าจ้างล่วงหน้าให้แก่ที่ปรึกษาต่อเมื่อผู้ว่าจ้างได้หักเงินค่าจ้างไว้จนครบจำนวนเงินค่าจ้างล่วงหน้าตามข้อ ๔.๓ แล้ว
    </div>
   <div class='tab3 t-16'><b>ข้อ 5 ความรับผิดชอบของที่ปรึกษา</b></div>
    <div class='tab3 t-16'>
        5.1.ที่ปรึกษาจะต้องส่งมอบผลงานตามรูปแบบและวิธีการ {result.SendWorkMethod} จำนวน {result.WorkAmount} ชุด ให้แก่ผู้ว่าจ้างตามที่กำหนดไว้ในเอกสารแนบท้ายสัญญาผนวก1
    </div>
    <div class='tab3 t-16'>
        5.2.ในกรณีที่ผลงานของที่ปรึกษาบกพร่องหรือไม่เป็นไปตามข้อกำหนดและเงื่อนไข ตามสัญญาหรือมิได้ดำเนินการให้ถูกต้องตามหลักวิชาการ หรือวิชาชีพ {result.RelateExpertise} และ/หรือบทบัญญัติแห่งกฎหมายที่เกี่ยวข้อง ที่ปรึกษาต้องรีบทำการแก้ไขให้เป็นที่เรียบร้อย โดยไม่คิดค่าจ้าง ค่าเสียหาย หรือค่าใช้จ่ายใดๆ จากผู้ว่าจ้างอีก ถ้าที่ปรึกษาหลีกเลี่ยงหรือไม่รีบจัดการแก้ไขให้เป็นที่เรียบร้อยในกำหนดเวลาที่ผู้ว่าจ้างแจ้งเป็นหนังสือ ผู้ว่าจ้างมีสิทธิจ้างที่ปรึกษารายอื่นทำการแทน โดยที่ปรึกษาจะต้องรับผิดชอบจ่ายเงินค่าจ้างในการนี้แทนผู้ว่าจ้างโดยสิ้นเชิง
    </div>
    <div class='tab3 t-16'>
        ถ้ามีความเสียหายเกิดขึ้นจากงานตามสัญญานี้ไม่ว่าจะเนื่องมาจากการที่ที่ปรึกษา ได้ปฏิบัติงานไม่ถูกต้องตามหลักวิชาการ หรือวิชาชีพ{result.RelateExpertise}และ/หรือบทบัญญัติแห่งกฎหมายที่เกี่ยวข้อง หรือเหตุใด ที่ปรึกษาจะต้องทำการแก้ไขความเสียหายดังกล่าว ภายในเวลาที่ผู้ว่าจ้างกำหนดให้ ถ้าที่ปรึกษา ไม่สามารถแก้ไขได้ ที่ปรึกษาจะต้องชดใช้ค่าเสียหายที่เกิดขึ้นแก่ผู้ว่าจ้างโดยสิ้นเชิง ซึ่งรวมทั้งความเสียหายที่เกิดขึ้นโดยตรง และโดยส่วนที่เกี่ยวเนื่องกับความเสียหายที่เกิดขึ้นจากงานตามสัญญานี้ด้วย
    </div>
    <div class='tab3 t-16'>
        การที่ผู้ว่าจ้างได้ให้การรับรองหรือความเห็นชอบหรือความยินยอมใดๆ ในการปฏิบัติงานหรือผลงานของที่ปรึกษาหรือการชำระเงินค่าจ้างตามสัญญาแก่ที่ปรึกษา ไม่เป็นการปลดเปลื้องพันธะและ ความรับผิดชอบใดๆ ของที่ปรึกษาตามสัญญานี้
    </div>
    <div class='tab3 t-16'>
        5.3.บุคลากรหลักของที่ปรึกษา ต้องมีระยะเวลาปฏิบัติงานตามสัญญานี้ไม่ซ้ำซ้อนกับงานในโครงการอื่นๆ ของที่ปรึกษาที่ดำเนินการในช่วงเวลาเดียวกัน หากผู้ว่าจ้างพบว่าบุคลากรหลักไม่ว่าคนหนึ่งคนใดหรือหลายคนปฏิบัติงานซ้ำซ้อนกับงานในโครงการอื่นๆ ไม่ว่าจะพบในระหว่างปฏิบัติงานตามสัญญาหรือในภายหลัง ผู้ว่าจ้างมีสิทธิบอกเลิกสัญญา และ/หรือเรียกค่าเสียหายจากที่ปรึกษาหรือปรับลดค่าจ้างได้
    </div>
    <div class='tab3 t-16'><b>ข้อ 6 การระงับการทำงานชั่วคราวและการบอกเลิกสัญญา</b></div>
    <div class='tab3 t-16'>
        6.1.ผู้ว่าจ้างมีสิทธิบอกเลิกสัญญาในกรณีดังต่อไปนี้
    </div>
    <div class='tab3 t-16'>
        (ก)หากผู้ว่าจ้างเห็นว่าที่ปรึกษามิได้ปฏิบัติงานด้วยความชำนาญหรือด้วยความเอาใจใส่ในวิชาชีพของที่ปรึกษาเท่าที่พึงคาดหมายได้จากที่ปรึกษาในระดับเดียวกัน หรือมิได้ปฏิบัติตามสัญญาข้อใดข้อหนึ่ง ในกรณีเช่นนี้ผู้ว่าจ้างจะบอกกล่าวให้ที่ปรึกษาทราบถึงเหตุผลที่จะบอกเลิกสัญญา ถ้าที่ปรึกษามิได้ดำเนินการแก้ไขให้ผู้ว่าจ้างพอใจภายในระยะเวลา {result.FixDaysAfterNoti} วัน นับถัดจากวันที่ได้รับคำบอกกล่าว ผู้ว่าจ้างมีสิทธิบอกเลิกสัญญาโดยการส่งคำบอกกล่าวแก่ที่ปรึกษา เมื่อที่ปรึกษาได้รับหนังสือบอกกล่าวนั้นแล้ว ที่ปรึกษาต้องหยุดปฏิบัติงานทันที และดำเนินการทุกวิถีทางเพื่อลดค่าใช้จ่ายใดๆ ที่อาจมีในระหว่างการหยุดปฏิบัติงานนั้นให้น้อยที่สุด
    </div>
    <div class='tab3 t-16'>
        (ข)หากที่ปรึกษามิได้ลงมือทำงานภายในกำหนดเวลา หรือไม่สามารถทำงานให้แล้วเสร็จตามกำหนดเวลา หรือมีเหตุให้ผู้ว่าจ้างเชื่อได้ว่าที่ปรึกษาไม่สามารถทำงานให้แล้วเสร็จภายในกำหนดเวลา หรือล่วงเลยกำหนดเวลาแล้วเสร็จไปแล้ว หรือตกเป็นผู้ล้มละลาย ผู้ว่าจ้างมีสิทธิบอกเลิกสัญญาได้ทันที
    </div>
    <div class='tab3 t-16'>
        การบอกเลิกสัญญาตามข้อ ๖.๑ ผู้ว่าจ้างมีสิทธิริบหรือบังคับจากหลักประกันเงินค่าจ้างล่วงหน้า หลักประกันการปฏิบัติตามสัญญาและเงินประกันผลงานทั้งหมดหรือแต่บางส่วน และมีสิทธิเรียกค่าเสียหายอื่น (ถ้ามี)จากที่ปรึกษาด้วย
    </div>
    <div class='tab3 t-16'>
        6.2.ผู้ว่าจ้างอาจมีหนังสือบอกกล่าวให้ที่ปรึกษาทราบล่วงหน้าเมื่อใดก็ได้ว่าผู้ว่าจ้างมีเจตนาที่จะระงับการทำงานของที่ปรึกษาไว้ชั่วคราวไม่ว่าทั้งหมดหรือแต่บางส่วน หรือจะบอกเลิกสัญญา ในกรณีที่ผู้ว่าจ้าง จะบอกเลิกสัญญา การบอกเลิกสัญญาดังกล่าวจะมีผลในเวลาไม่น้อยกว่า {result.NotiDaysAfterTerminate} วัน นับถัดจากวันที่ที่ปรึกษาได้รับหนังสือบอกกล่าวนั้น หรืออาจเร็วกว่าหรือช้ากว่ากำหนดเวลานั้นก็ได้แล้วแต่คู่สัญญาจะทำความตกลงกัน เมื่อที่ปรึกษาได้รับหนังสือบอกกล่าวนั้นแล้ว ที่ปรึกษาต้องหยุดปฏิบัติงานทันที ทั้งนี้ ที่ปรึกษาไม่มีสิทธิได้รับค่าจ้างในระหว่างระงับการทำงานไว้ชั่วคราว และที่ปรึกษาจะต้องดำเนินการทุกวิถีทางเพื่อลดค่าใช้จ่ายใดๆ ที่อาจมีในระหว่างการหยุดปฏิบัติงานนั้นให้น้อยที่สุด
    </div>
    <div class='tab3 t-16'>
        กรณีที่มีการระงับการทำงานชั่วคราวตามข้อ ๖.๒ ผู้ว่าจ้างจะจ่ายเงินเป็นค่าใช้จ่ายเท่าที่จำเป็นให้แก่ที่ปรึกษาตามที่ผู้ว่าจ้างเห็นสมควร
    </div>
    <div class='tab3 t-16'>
        กรณีที่มีการเลิกสัญญาตามข้อ ๖.๒ ผู้ว่าจ้างจะชำระค่าจ้างตามส่วนที่เป็นธรรมและเหมาะสมที่กำหนดในเอกสารแนบท้ายสัญญาผนวก ......ให้แก่ที่ปรึกษา โดยคำนวณตั้งแต่วันเริ่มปฏิบัติงานจนถึงวันบอกเลิกสัญญา นอกจากนี้ผู้ว่าจ้างจะคืนหลักประกันการปฏิบัติตามสัญญาหรือเงินประกันผลงาน รวมทั้งเงินชดเชยค่าเดินทางและเงินค่าใช้จ่ายที่ได้ทดรองจ่ายไปตามสมควรและตามความเป็นจริง ซึ่งผู้ว่าจ้างยังมิได้ชำระให้แก่ที่ปรึกษาด้วย อย่างไรก็ตาม เงินชดเชยและเงินที่ได้ชำระไปแล้วทั้งหมดจะต้องไม่เกินราคาค่าจ้างตามข้อ ๓
    </div>
    <div class='tab3 t-16'><b>ข้อ 7 สิทธิและหน้าที่ของที่ปรึกษา</b></div>
    <div class='tab3 t-16'>
        7.1.ที่ปรึกษาจะต้องใช้ความชำนาญ ความระมัดระวัง และความขยันหมั่นเพียร ในการปฏิบัติงานตามสัญญาอย่างมีประสิทธิภาพ และจะต้องปฏิบัติหน้าที่ตามความรับผิดชอบให้สำเร็จลุล่วง เป็นไปตามมาตรฐานของวิชาชีพที่ยอมรับนับถือกันโดยทั่วไป
    </div>
    <div class='tab3 t-16'>
        7.2.ค่าจ้างซึ่งผู้ว่าจ้างจะชำระแก่ที่ปรึกษาตามข้อ ๓ เป็นค่าตอบแทนเพียงอย่างเดียวเท่านั้นซึ่งที่ปรึกษาจะได้รับเกี่ยวกับการปฏิบัติงานตามสัญญานี้ ที่ปรึกษาจะต้องไม่รับค่านายหน้าทางการค้า ส่วนลด เบี้ยเลี้ยง เงินช่วยเหลือหรือผลประโยชน์ใดๆ ไม่ว่าโดยตรงหรือโดยอ้อม หรือสิ่งตอบแทนใดๆในส่วนที่เกี่ยวข้องกับสัญญานี้ หรือที่เกี่ยวกับการปฏิบัติหน้าที่ตามสัญญานี้
    </div>
    <div class='tab3 t-16'>
        7.3.ที่ปรึกษาจะต้องไม่มีผลประโยชน์ใดๆ ไม่ว่าโดยตรงหรือโดยอ้อมในเงินค่าสิทธิ เงินบำเหน็จ หรือค่านายหน้าใดๆ ที่เกี่ยวกับการนำวัสดุสิ่งของหรือกรรมวิธีใดๆ ที่มีทะเบียนสิทธิบัตรหรือได้รับการคุ้มครองทางทรัพย์สินทางปัญญาหรือตามกฎหมายอื่นใดมาใช้เพื่อวัตถุประสงค์ของสัญญานี้ เว้นแต่คู่สัญญา ทั้งสองฝ่ายจะได้ตกลงกันเป็นหนังสือว่าที่ปรึกษาอาจจะได้ผลประโยชน์หรือเงินเช่นว่านั้นได้
    </div>
    <div class='tab3 t-16'>
        7.4.บรรดางานและเอกสารที่ที่ปรึกษาได้จัดทำขึ้นเกี่ยวกับสัญญานี้ให้ถือเป็นความลับและให้ตกเป็นกรรมสิทธิ์ของผู้ว่าจ้าง ที่ปรึกษาจะต้องส่งมอบบรรดางานและเอกสารดังกล่าวให้แก่ผู้ว่าจ้างเมื่อสิ้นสุดสัญญานี้ ที่ปรึกษาอาจเก็บสำเนาเอกสารไว้กับตนได้แต่ต้องไม่นำข้อความในเอกสารนั้นไปใช้ในกิจการอื่นที่ ไม่เกี่ยวกับงานโดยไม่ได้รับความยินยอมล่วงหน้าเป็นหนังสือจากผู้ว่าจ้างก่อน
    </div>
    <div class='tab3 t-16'>
        7.5.ผู้ว่าจ้างเป็นเจ้าของลิขสิทธิ์หรือสิทธิในทรัพย์สินทางปัญญา รวมถึงสิทธิใดๆ ในผลงานที่ที่ปรึกษาได้ปฏิบัติงานตามสัญญานี้แต่เพียงฝ่ายเดียว และที่ปรึกษาจะนำผลงาน และ/หรือรายละเอียดของงานตามสัญญานี้ ไม่ว่าทั้งหมดหรือบางส่วนไปใช้ หรือเผยแพร่ในกิจการอื่น นอกเหนือจากที่ได้ระบุไว้ในสัญญานี้ไม่ได้ เว้นแต่ได้รับอนุญาตเป็นหนังสือจากผู้ว่าจ้างก่อน
    </div>
    <div class='tab3 t-16'>
        7.6.บรรดาเครื่องมือ เครื่องใช้ และวัสดุอุปกรณ์ทั้งหลาย ซึ่งผู้ว่าจ้างได้จัดให้ที่ปรึกษาใช้หรือซึ่งที่ปรึกษาซื้อมาด้วยทุนทรัพย์ของผู้ว่าจ้าง หรือซึ่งผู้ว่าจ้างเป็นผู้จ่ายชดใช้คืนให้ ถือว่าเป็นกรรมสิทธิ์ของผู้ว่าจ้างและต้องทำข้อความและเครื่องหมายที่แสดงว่าเป็นของผู้ว่าจ้างไว้ที่ทรัพย์สินดังกล่าวด้วย ทั้งนี้ ที่ปรึกษาต้องใช้เครื่องมือเครื่องใช้และวัสดุอุปกรณ์ดังกล่าวอย่างเหมาะสมตามระเบียบของผู้ว่าจ้างหรือของทางราชการ ที่เกี่ยวข้องเพื่อกิจการที่เกี่ยวกับการจ้างที่ปรึกษาเท่านั้น
    </div>
    <div class='tab3 t-16'>
        เมื่อที่ปรึกษาทำงานเสร็จหรือมีการเลิกสัญญา ที่ปรึกษาจะต้องทำบัญชีแสดงรายการเครื่องมือเครื่องใช้และวัสดุอุปกรณ์ทั้งหลายข้างต้นที่ยังคงเหลืออยู่ และจัดการโยกย้ายไปเก็บรักษาตามคำสั่งผู้ว่าจ้างโดยพลัน ที่ปรึกษาต้องดูแลเครื่องมือเครื่องใช้และวัสดุอุปกรณ์ดังกล่าวอย่างเหมาะสมตลอดเวลาที่ครอบครอง และต้องคืนเครื่องมือเครื่องใช้และวัสดุอุปกรณ์ดังกล่าวให้ครบถ้วนในสภาพดีตามความเหมาะสม แต่ไม่ต้องรับผิดชอบสำหรับความเสื่อมสภาพจากการใช้งานตามปกติ
    </div>
    <div class='tab3 t-16'>
        7.7.ที่ปรึกษาจะจัดให้มีบุคลากรที่มีความรู้และความชำนาญงานมาปฏิบัติงานให้เหมาะสมกับสภาพการปฏิบัติงานตามสัญญานี้และให้สอดคล้องกับขอบเขตของงานของที่ปรึกษาตามที่ปรากฏ ในเอกสารแนบท้ายสัญญาผนวก …..การเปลี่ยนแปลงบุคลากรดังกล่าวจะต้องได้รับความยินยอมเป็นหนังสือจากผู้ว่าจ้างก่อน
    </div>
    <div class='tab3 t-16'>
        7.8.ในกรณีที่ผู้ว่าจ้างพิจารณาเห็นว่า การดำเนินงานของบุคลากรที่ที่ปรึกษาจัดหามาจะก่อให้เกิดความเสียหายแก่งานตามสัญญานี้ ไม่ว่าในกรณีใดก็ตามผู้ว่าจ้างมีสิทธิที่จะให้ที่ปรึกษาเปลี่ยนบุคลากรบางคนหรือทั้งหมดนั้นได้ และที่ปรึกษาต้องดำเนินการตามความประสงค์ของผู้ว่าจ้างโดยเร็ว
    </div>
    <div class='tab3 t-16'>
        การเปลี่ยนบุคลากรตามความในวรรคก่อน ที่ปรึกษาจะต้องเสนอรายชื่อบุคลากรที่จะปฏิบัติงานแทนนั้น ต่อผู้ว่าจ้างเพื่อพิจารณาให้ความเห็นชอบก่อน
    </div>
    <div class='tab3 t-16'><b>ข้อ 8 ความรับผิดชอบของที่ปรึกษาต่อบุคคลภายนอก</b></div>
    <div class='tab3 t-16'>
        8.1.ที่ปรึกษาจะต้องชดใช้ค่าเสียหายให้แก่ผู้ว่าจ้าง และป้องกันมิให้ผู้ว่าจ้างต้องรับผิดชอบในบรรดาสิทธิเรียกร้อง ค่าเสียหาย ค่าใช้จ่าย หรือราคา รวมตลอดถึงการเรียกร้องโดยบุคคลภายนอกอันเกิดจากความผิดพลาดหรือการละเว้นไม่กระทำการของที่ปรึกษา หรือของลูกจ้างของที่ปรึกษา
    </div>
    <div class='tab3 t-16'>
        8.2.ที่ปรึกษาจะต้องรับผิดชอบต่อการละเมิดบทบัญญัติแห่งกฎหมาย หรือการละเมิดลิขสิทธิ์ หรือสิทธิในทรัพย์สินทางปัญญาอื่น รวมถึงสิทธิใดๆ ต่อบุคคลภายนอกเนื่องจากการปฏิบัติงานตามสัญญานี้ โดยสิ้นเชิง
    </div>
    <div class='tab3 t-16'>
        (9)8.3.ที่ปรึกษาจะต้องจัดการประกันภัยกับบริษัทประกันภัยที่ผู้ว่าจ้างเห็นชอบเพื่อความรับผิดต่อบุคคลภายนอก และเพื่อความสูญหายหรือเสียหายในทรัพย์สินซึ่งผู้ว่าจ้างเป็นผู้จัดหาให้หรือสั่งซื้อโดยทุนทรัพย์ของผู้ว่าจ้าง เพื่อให้ที่ปรึกษาไว้ใช้ในการปฏิบัติงานตามสัญญานี้ โดยที่ปรึกษาเป็นผู้ออกค่าใช้จ่ายในการประกันภัยเอง ทั้งนี้ เว้นแต่จะมีการตกลงกันไว้เป็นอย่างอื่นในสัญญานี้
    </div>
    <div class='tab3 t-16'><b>ข้อ 9 พันธะหน้าที่ของผู้ว่าจ้าง</b></div>
    <div class='tab3 t-16'>
        ผู้ว่าจ้างจะมอบข้อมูลและสถิติต่างๆ ที่เกี่ยวข้องซึ่งผู้ว่าจ้างมีอยู่ให้แก่ที่ปรึกษาโดยไม่คิดมูลค่าและภายในเวลาอันควร
    </div>
    <div class='tab3 t-16'>
        ในกรณีที่ที่ปรึกษาร้องขอความช่วยเหลือ ผู้ว่าจ้างจะพิจารณาให้ความช่วยเหลืออำนวยความสะดวกตามสมควร ทั้งนี้ เพื่อให้การปฏิบัติงานของที่ปรึกษาตามสัญญานี้ลุล่วงไปได้ด้วยดี
    </div>
    <div class='tab3 t-16'><b>ข้อ 10 ค่าปรับ</b></div>
    <div class='tab3 t-16'>
        ในกรณีที่ที่ปรึกษาไม่ปฏิบัติตามสัญญาข้อใดข้อหนึ่งด้วยเหตุใดๆ ก็ตาม จนเป็นเหตุให้เกิดค่าปรับ ค่าเสียหาย หรือค่าใช้จ่ายแก่ผู้ว่าจ้าง ที่ปรึกษาต้องชดใช้ค่าปรับ ค่าเสียหาย หรือค่าใช้จ่ายดังกล่าวให้แก่ผู้ว่าจ้างโดยสิ้นเชิงภายในกำหนด..........................(............................)วัน นับถัดจากวันที่ได้รับแจ้งเป็นหนังสือจากผู้ว่าจ้าง หากที่ปรึกษาไม่ชดใช้ให้ถูกต้องครบถ้วนภายในระยะเวลาดังกล่าว ให้ผู้ว่าจ้างมีสิทธิที่จะหักเอาจากจำนวนเงินค่าจ้างของที่ปรึกษาที่ต้องชำระ หรือบังคับจากหลักประกันการปฏิบัติตามสัญญา หรือเงินประกันผลงานได้ทันที
    </div>
    <div class='tab3 t-16'>
        หากค่าปรับ ค่าเสียหาย หรือค่าใช้จ่ายที่บังคับจากเงินค่าจ้างที่ต้องชำระ หลักประกัน การปฏิบัติตามสัญญา และเงินประกันผลงานแล้วยังไม่เพียงพอ ที่ปรึกษายินยอมชำระส่วนที่เหลือที่ยังขาดอยู่จนครบถ้วนตามจำนวนค่าปรับ ค่าเสียหาย หรือค่าใช้จ่ายนั้น ภายในกำหนด..................(....................)วันนับถัดจากวันที่ได้รับแจ้งเป็นหนังสือจากผู้ว่าจ้าง
    </div>
    <div class='tab3 t-16'>
        หากมีเงินค่าจ้างตามสัญญาที่หักไว้จ่ายเป็นค่าปรับ ค่าเสียหาย หรือค่าใช้จ่ายแล้วยังเหลืออยู่อีกเท่าใด ผู้ว่าจ้างจะคืนให้แก่ที่ปรึกษาทั้งหมด
    </div>
    <div class='tab3 t-16'><b>(11)ข้อ 12 (ก)เงินประกันผลงาน</b></div>
    <div class='tab3 t-16'>
        ในการจ่ายเงินให้แก่ที่ปรึกษาแต่ละงวด ผู้ว่าจ้างจะหักเงินจำนวนร้อยละ…(12)….(................)ของเงินที่ต้องจ่ายในงวดนั้นเพื่อเป็นประกันผลงาน หรือที่ปรึกษาอาจนำหนังสือค้ำประกันของธนาคารหรือหนังสือค้ำประกันอิเล็กทรอนิกส์ของธนาคารภายในประเทศซึ่งมีอายุการค้ำประกันตลอดอายุสัญญามามอบให้ผู้ว่าจ้าง ทั้งนี้เพื่อเป็นหลักประกันแทนก็ได้
    </div>
    <div class='tab3 t-16'>
        ผู้ว่าจ้างจะคืนเงินประกันผลงาน และ/หรือหนังสือค้ำประกันของธนาคารดังกล่าวตามวรรคหนึ่งโดยไม่มีดอกเบี้ยให้แก่ที่ปรึกษาพร้อมกับการจ่ายเงินค่าจ้างงวดสุดท้าย
    </div>
    <div class='tab3 t-16'><b>ข้อ ๑๒ (ข)หลักประกันการปฏิบัติตามสัญญา</b></div>
    <div class='tab3 t-16'>
        ในขณะทำสัญญานี้ที่ปรึกษาได้นำหลักประกันเป็น……........(13)….….....….เป็นจำนวนเงิน…..……….……บาท (………………………)ซึ่งเท่ากับร้อยละ.…(14)....(……………….......)ของราคาค่าจ้างตามสัญญา มามอบไว้แก่ผู้ว่าจ้างเพื่อเป็นหลักประกันการปฏิบัติตามสัญญานี้
    </div>
    <div class='tab3 t-16'>
        (15)กรณีที่ปรึกษาใช้หนังสือค้ำประกันมาเป็นหลักประกันการปฏิบัติตามสัญญา หนังสือค้ำประกันดังกล่าวจะต้องออกโดยธนาคารที่ประกอบกิจการในประเทศไทย หรือโดยบริษัทเงินทุนหรือบริษัทเงินทุนหลักทรัพย์ที่ได้รับอนุญาตให้ประกอบกิจการเงินทุนเพื่อการพาณิชย์และประกอบธุรกิจค้ำประกันตามประกาศของธนาคารแห่งประเทศไทย ตามรายชื่อบริษัทเงินทุนที่ธนาคารแห่งประเทศไทยแจ้งเวียนให้ทราบ โดยอนุโลมให้ใช้ตามตัวอย่างหนังสือค้ำประกันของธนาคารที่คณะกรรมการนโยบายกำหนด หรืออาจเป็นหนังสือค้ำประกันอิเล็กทรอนิกส์ตามวิธีการที่กรมบัญชีกลางกำหนดก็ได้ และจะต้องมีอายุการค้ำประกันตลอดไปจนกว่าที่ปรึกษาพ้นข้อผูกพันตามสัญญานี้
    </div>
    <div class='tab3 t-16'>
        หลักประกันจะต้องมีอายุครอบคลุมความรับผิดทั้งปวงของที่ปรึกษาตลอดอายุสัญญา ถ้าหลักประกันที่ที่ปรึกษานำมามอบให้ดังกล่าวลดลงหรือเสื่อมค่าลง หรือมีอายุไม่ครอบคลุมถึงความรับผิดของ ที่ปรึกษาตลอดอายุสัญญา ไม่ว่าด้วยเหตุใดๆ ก็ตาม รวมถึงกรณีที่ปรึกษาส่งมอบงานล่าช้าเป็นเหตุให้ระยะเวลา แล้วเสร็จตามสัญญาเปลี่ยนแปลงไป ที่ปรึกษาต้องหาหลักประกันใหม่หรือหลักประกันเพิ่มเติมให้มีจำนวนครบถ้วน ตามวรรคหนึ่งมามอบให้แก่ผู้ว่าจ้างภายใน.............(………..….)วัน นับถัดจากวันที่ได้รับแจ้งเป็นหนังสือจากผู้ว่าจ้าง
    </div>
    <div class='tab3 t-16'>
        หลักประกันที่ผู้รับจ้างนำมามอบไว้ตามข้อนี้ ผู้ว่าจ้างจะคืนให้แก่ที่ปรึกษาโดยไม่มีดอกเบี้ย เมื่อที่ปรึกษาพ้นจากข้อผูกพันและความรับผิดทั้งปวงตามสัญญานี้แล้ว
    </div>
    <div class='tab3 t-16'><b>ข้อ ๑๓ การจ้างช่วง</b></div>
    <div class='tab3 t-16'>
        ที่ปรึกษาจะต้องไม่โอนสิทธิประโยชน์ใดๆ ตามสัญญานี้ให้แก่ผู้อื่นโดยไม่ได้รับความยินยอมเป็นหนังสือจากผู้ว่าจ้างก่อน เว้นแต่การโอนสิทธิที่จะรับเงินค่าจ้างตามสัญญานี้
    </div>
    <div class='tab3 t-16'><b>ข้อ ๑๕ การงดหรือลดค่าปรับ หรือขยายเวลาปฏิบัติงานตามสัญญา</b></div>
    <div class='tab3 t-16'>
        ในกรณีที่มีเหตุเกิดจากความผิดหรือความบกพร่องของผู้ว่าจ้างหรือเหตุสุดวิสัย หรือเกิดจากพฤติการณ์อันหนึ่งอันใดที่ที่ปรึกษาไม่ต้องรับผิดตามกฎหมาย หรือเหตุอื่นตามที่กำหนดในกฎกระทรวง ซึ่งออกตามความในกฎหมายว่าด้วยการจัดซื้อจัดจ้างและการบริหารพัสดุภาครัฐ ทำให้ที่ปรึกษาไม่สามารถทำงานให้แล้วเสร็จตามเงื่อนไขและกำหนดเวลาแห่งสัญญานี้ได้ ที่ปรึกษาจะต้องแจ้งเหตุหรือพฤติการณ์ดังกล่าว พร้อมหลักฐานเป็นหนังสือให้ผู้ว่าจ้างทราบ เพื่อของดหรือลดค่าปรับ หรือขยายเวลาทำงานออกไปภายใน ๑๕ (สิบห้า)วัน นับถัดจากวันที่เหตุนั้นสิ้นสุดลง หรือตามที่กำหนดในกฎกระทรวงดังกล่าว แล้วแต่กรณี
    </div>
    <div class='tab3 t-16'>
        ถ้าที่ปรึกษาไม่ปฏิบัติให้เป็นไปตามความในวรรคหนึ่ง ให้ถือว่าที่ปรึกษาได้สละสิทธิเรียกร้อง ในการที่จะของดหรือลดค่าปรับ หรือขยายเวลาทำงานออกไปโดยไม่มีเงื่อนไขใดๆ ทั้งสิ้น เว้นแต่กรณีเหตุเกิดจากความผิดหรือความบกพร่องของฝ่ายผู้ว่าจ้างซึ่งมีหลักฐานชัดแจ้งหรือผู้ว่าจ้างทราบดีอยู่แล้วตั้งแต่ต้น
    </div>
    <div class='tab3 t-16'>
...
<head>
    <meta charset='utf-8'>
     ...
     <style>
        ...
        .tab3 {{ text-indent: 48px;  word-break: break-all;  }}
